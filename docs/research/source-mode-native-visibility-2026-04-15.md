...
> **DEPRECATED 2026-04-18 by [ADR-001: D-3seg Versioning, Package-First Local Dev, Artifact Source Profile Abstraction](../decisions/2026-04-18-versioning-d3seg.md).** The entire consumer-side Source Mode mechanism described in this document (ProjectReference-based MSBuild Content-injection from `src/native/<lib>/runtimes/<rid>/native/` staging into consumer `bin/.../runtimes/<rid>/native/`) is retired. Under ADR-001, all consumer-facing csprojs (smoke, examples, sandbox, future samples) consume via `PackageReference` against a local folder feed — the unified package-first consumer contract. Feed preparation is handled by the new `SetupLocalDev` Cake task (`--source=local` implemented Phase 2a; `--source=remote` Phase 2b), and the per-developer consumer override lives in `Janset.Smoke.local.props` (gitignored).
>
> **What stays useful:** the symlink-preservation findings in §5.1 (WSL Ubuntu) and §5.2 (Intel Mac) remain informational for any future remote-feed tar-extract caching work under `RemoteInternalArtifactSourceResolver`. The empirical measurements (`<Content>`-only 3× duplication + chain flattening vs `<Exec cp -a>` 1× preservation) are solid data, even though the consumer-side mechanism they designed is no longer adopted.
>
> **What is retired:** Candidate B (solution-root `Directory.Build.targets` + opt-in `$(JansetSdl2SourceMode)` flag), `Source-Mode-Prepare` Cake task concept, `artifacts/native-staging/<rid>/native/*` consumer-staging layout, `test/Directory.Build.props` Source-Mode preset, PD-6 (net462 source-mode visibility — now N/A under package-first).
>
> **PD-4 status:** CLOSED. **PD-5 status:** REFRAMED as `RemoteInternal` Artifact Source Profile implementation (see [ADR-001 §2.7](../decisions/2026-04-18-versioning-d3seg.md) and [phase-2-adaptation-plan.md](../phases/phase-2-adaptation-plan.md) Pending Decisions).
>
> Read the rest of this document as historical context. Current execution model: [ADR-001](../decisions/2026-04-18-versioning-d3seg.md).

---

**Date:** 2026-04-15
**Status:** **DEPRECATED 2026-04-18** (see banner above). Original status: Mechanism locked — the **copy mechanism** (staging → `bin/`) is empirically verified on Windows (worktree, 2026-04-15), Linux (WSL Ubuntu, 2026-04-15), and macOS (SSH Intel Mac, Darwin 24.6.0, 2026-04-15). "Verified" here means "the MSBuild-level mechanism delivers the right files to `bin/` with the right shape and correct platform gating." It does **not** mean "real SDL2 natives load at runtime under dyld / ld.so with correct install-names and any code-signing baggage" — that is end-to-end validation, which is pending Stream F implementation with real SDL2 natives. Resolved Pending Decision **PD-4** (now closed by ADR-001). **PD-5 direction** (non-host RID acquisition via `--source=remote`) was locked in §7.2 but its concrete mechanism (URL convention, producer workflow, archive contract, auth, caching) was unresolved; ADR-001 reframes the remote path as RemoteInternal profile implementation under the package-first contract.
