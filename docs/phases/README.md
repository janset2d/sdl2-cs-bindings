...
**Phase X: Build-Host Modernization — ADR-002 target-centric refactor.** ADR-004 migration is closed (P0 → P4-A on master). ADR-002 is the active continuation; it absorbs residual ADR-004 cleanup. Canonical docs: [`../decisions/2026-05-05-target-centric-build-host.md`](../decisions/2026-05-05-target-centric-build-host.md), [`../refactoring/target-centric-build-host-refactor-plan.md`](../refactoring/target-centric-build-host-refactor-plan.md), [`../refactoring/target-centric-build-host-review-checklist.md`](../refactoring/target-centric-build-host-review-checklist.md). P0 → P9 closed (S00 → S15, last update 2026-05-10): docs/guardrails landed, V2 test infra + foundation primitives shipped, repositories + named `BuildContext` properties wired, `Info` + `ResolveVersions{FromManifest,FromExplicit}` + four diagnostic targets migrated, coverage gate + strategy abstraction retired, PreFlight migrated to `Targets/PreFlightCheck/` with root-level `Validation/` named concept (S12), Pack migrated to `Targets/Package/` with `PackageFamilyPacker` owning the per-family 3-phase flow + 3 more OneOf result types retired (S13), Harvest stack migrated to `Targets/{Harvest,NativeSmoke,ConsolidateHarvest}/` with 3 reporters sharing the IAnsiConsole + ICakeLog cohort pattern + 3 OneOf result types retired + `Features/Harvesting/` folder dissolved (S14), **PackageConsumerSmoke + PublishStaging + PublishPublic migrated to `Targets/`; last surviving OneOf (`PackageInfoResult`) retired + OneOf NuGet dep removed entirely; Vcpkg promoted to root `Build.Vcpkg` named concept; 3 Configuration classes retired (`DotNetBuildConfiguration` + `VcpkgConfiguration` + `PackageBuildConfiguration` → direct `PackageFamilyVersionSet` DI); Pack-finish relocations (NativePackageMetadata + ReadmeMappingTable + FamilyTopologyHelpers + PackageArtifacts → `Targets/Package/{Models,Services}/`); `Features/{Packaging,Publishing}/` folders dissolved entirely (S15)**. Cross-platform validated 8/8 PASS on Windows + WSL/linux-x64 + macOS/osx-x64 (S12); local ci-sim 8/8 PASS in 177s on Windows post-S13. Test count: 525 → 597. Next: P10 (Shared/* promotion + Features/Ci+Vcpkg+DependencyAnalysis+Diagnostics migrations + remaining Integrations/* relocation + final doc sweep).
