...
**Status:** IN PROGRESS — ADR-003 implementation and CI/CD rewrite are landed; remaining work is the Phase 2b tail (PA-2 witness, remote artifact source, first prerelease publication path).
**Prerequisites:**
...
- PA-2 behavioral witness runs on `win-arm64`, `win-x86`, `linux-arm64`, `osx-arm64` via the live pipeline.
...
3. **CI/CD workflow rewrite** *(landed through Slice E follow-up pass)* — `release.yml` grew through every slice: stub `resolve-versions-stub` (A) → real `resolve-versions` + `preflight` (B1) → `generate-matrix` + first-draft `harvest` / `native-smoke` / `consolidate-harvest` (D) → real `pack` + `consumer-smoke` matrix re-entry + `publish-staging` / `publish-public` stubs (C) → `build-cake-host` single-runner FDD publish + all consumers consume `dotnet ./cake-host/Build.dll` instead of per-job restore+build (E1a-b) → composite-action absorption, lock-file discipline, workflow retirement, publish stubs, and three-platform witness closure in the Slice E follow-up pass. `docker/linux-builder.Dockerfile` and `.github/workflows/build-linux-container.yml` now back the live multi-arch GHCR Linux builder image referenced from `manifest.runtimes[]`. The remaining work is operational: PA-2 witness evidence, remote artifact sourcing, and real publish-stage transfer.
4. **PA-2 behavioral validation via the live pipeline** *(pending — Phase 2b tail)* — `workflow_dispatch` on `win-arm64` / `win-x86` / `linux-arm64` / `osx-arm64`; `mode=manifest-derived`, `suffix=pa2.<run-id>`; failure triage per playbook PA-2 section. Infrastructure blockers are closed; the missing piece is the witness run and its evidence trail.
...
| **PA-2 (behavioral)** | End-to-end pack + consumer smoke on the four newly-hybridized rows | **Pending.** Runs through the live `release.yml`; workflow and GHCR infrastructure are already in place. |
