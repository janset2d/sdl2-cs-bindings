...
- Build-host test suite: **460 tests green** (2026-04-25 measurement; trajectory 340 → 355 → 390 → 398 → 400 → 418 → 426 → 460 across Slices A → E).
...
- PD-5 closed 2026-04-26 (commits `bc7c677` + `fdabcae` + `549ad2f`): `RemoteArtifactSourceResolver` + `INuGetFeedClient` / `NuGetProtocolFeedClient` make `SetupLocalDev --source=remote` operational; `PublishStagingTask` real impl pushes managed + native nupkg pairs to GitHub Packages via `PackageUpdateResource.Push`; `VersionsFileWriter` shared helper extracted so Local + Remote profiles emit identical `versions.json`; `smoke-witness.cs` gains `remote` mode. `release.yml` run 24962876812 (workflow_dispatch on `fdabcae` with `publish-staging=true`) published the first internal-feed wave: 5 families × 2 nupkgs = 10 nupkgs at `<UpstreamMajor>.<UpstreamMinor>.0-ci.24962876812.1`. Local Windows host witness via `tests/scripts/smoke-witness.cs remote` 3/3 PASS, ConsumerSmoke 35/35.

**Remaining Phase 2b tail:**

- `publish-staging` gate-lift — currently `if: github.event_name == 'workflow_dispatch' && inputs.publish-staging == true`; lift to enable tag-push auto-trigger now that the e2e flow is witnessed.
- `--source=remote` host-witness completion on Linux + macOS (PD-5 exit-criterion #4 final closure).
...
| **D-ci** | CI package-publish + smoke gate + internal feed push | **Internal-feed push landed 2026-04-26** (PD-5 / commit `fdabcae`): `release.yml` `publish-staging` job pushes managed + native nupkg pairs to GitHub Packages via `PackageUpdateResource.Push`; gated behind `workflow_dispatch.inputs.publish-staging=true` until tag-push promotion is approved. **Remaining**: gate-lift, PD-7 nuget.org promotion (`Promote-To-Public.yml` or equivalent) + Trusted Publishing OIDC, PD-8 manual escape hatch. |
...
| **PD-5** | `RemoteInternal` Artifact Source Profile concrete implementation (reframed under ADR-001) | **Resolved 2026-04-26** (commits `bc7c677`, `fdabcae`, `549ad2f`). | Internal feed URL `https://nuget.pkg.github.com/janset2d/index.json` (hard-coded constant in `RemoteArtifactSourceResolver` + `PublishStagingTask`); auth via `GH_TOKEN → GITHUB_TOKEN` env chain (Classic PAT with `read:packages`; CI maps `secrets.GITHUB_TOKEN → env: GH_TOKEN`); cache strategy = `artifacts/packages/` reused across profiles with wipe-on-prepare; `SetupLocalDev --source=remote` operational on `win-x64` (`tests/scripts/smoke-witness.cs remote` 3/3 PASS, ConsumerSmoke 35/35); Linux + macOS host witnesses pending for full PD-5 exit-criterion #4 closure. | — |
