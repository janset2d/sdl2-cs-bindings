...
| **P4** — ConsumerSmoke + Pack + other consumers (E1c tail) | **DONE** | *filled at commit time* | Every non-harvest consumer job (`resolve-versions`, `preflight`, `generate-matrix`, `consolidate-harvest`, `pack`, `consumer-smoke`) gains `./.github/actions/nuget-cache` for uniform env + cross-run cache. `consumer-smoke` also gains `./.github/actions/platform-build-prereqs` (macOS brew at runtime just-in-case; Linux + Windows no-op). No vcpkg-setup on these — consumer-side Cake host only does NuGet restore / pack / test, never vcpkg invocation. `pack` + `consumer-smoke` retain the `mapfile -t args < <(jq ...)` expansion from versions.json artifact. |
...
| **P4c** — CI run observations (run 24786888297, 2026-04-22) | **PARTIAL GREEN** | (observation, not a commit) | Post-`eecc921` `workflow_dispatch` result: build-cake-host + resolve-versions + generate-matrix + preflight ✓. Harvest **7/7 ✓** (GCC 11 fixed libyuv + libwebp linker). ConsolidateHarvest ✓. Pack ✓. NativeSmoke **6/7 ✓** — only win-arm64 fails, and NOT at configure / build (both pass with Kitware cmake + PA-2 presets + MSVC `x64_arm64` cross-compile vcvars). The failure is at the *run the binary* step: `native-smoke.exe` compiled for ARM64 cannot run on the Windows x64 runner — `Error: An error occurred trying to start process '...native-smoke.exe' ... The specified executable is not a valid application for this OS platform`. ConsumerSmoke **0/7** — all seven RIDs fail (details in priming prompt; likely package feed path / restore / TUnit assertion surface). Publish (Staging / Public) remain gated `if: false`. |
