...
| **P5** — Lock-file discipline (E1d) | not started | — | `<RestorePackagesWithLockFile>true</RestorePackagesWithLockFile>` in `Build.csproj` + `Build.Tests.csproj` + `src/**/*.csproj` as applicable + `packages.lock.json` files generated and committed + CI restores use `--locked-mode`. |
| **P6** — PublishTask stubs (E4) | not started | — | `build/_build/Application/Publishing/PublishTaskRunner.cs` + `build/_build/Tasks/Publish/PublishTask.cs`: body throws `NotImplementedException` with Phase-2b message. `release.yml`'s `publish-staging` / `publish-public` jobs already gated `if: false`. |
| **P7** — Three-platform witness (E7) | not started | — | `workflow_dispatch` runs per `docs/playbook/cross-platform-smoke-validation.md` A-K checkpoints: Windows + WSL + macOS Intel SSH; all 7 RIDs green on harvest + native-smoke + consolidate-harvest + pack + consumer-smoke end-to-end. End-of-follow-up-pass gate. |
| **P8** — Retirement + doc sweep tail (E3 + E6) | not started | — | Delete `.github/workflows/prepare-native-assets-{linux,macos,windows,main}.yml` + `.github/workflows/release-candidate-pipeline.yml` (placeholder stub). Canonical-doc broader sweep tail: `AGENTS.md`, `onboarding.md`, `knowledge-base/cake-build-architecture.md` surface drift, §11 Q17 ADR-002 §2.3 delegate-hook stance resolution, **§11 Q18 CMakePresets developer-experience refactor** (drop `-interactive` preset variants + add `CMakeUserPresets.json.example` template + `tests/smoke-tests/native-smoke/README.md` + `.gitignore` entry). **Slice E follow-up pass closure commit.** |
