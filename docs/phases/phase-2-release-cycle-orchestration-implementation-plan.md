...
### 2.1 Progress log (`feat/adr003-impl` — merged to `master` 2026-04-22 at `bfc6713`)
...
| **Slice C (partial: C.1–C.8a)** | **DONE** | *filled at commit time* | Eight of thirteen sub-steps landed in one mid-slice commit (Deniz direction 2026-04-22): C.1 seven per-stage request records under `Domain/<Module>/Models/`; C.2 runner signature uniform cut-over to `Task RunAsync(BuildContext, TRequest, CT)` with `BuildContext.Runtime` added to keep Tasks ADR-002 §2.8 compliant; C.3 `Cake.Frosting.Git` 5.0.1 addin + `PackageTaskRunner.ResolveHeadCommitSha` migrated from inline `git rev-parse` subprocess to `context.GitLogTip().Sha` with a test-only delegate hook (Q17 tech-debt); C.4 `GitTagVersionProvider` (`Targeted` / `Train` sum-type scope; G54 per-entry; topological Train ordering via `FamilyTopologyHelpers`) + `Fixtures/TempGitRepo.cs` + 5 integration tests in new `Integration/Versioning/`; C.5 `ResolveVersionsTaskRunner` wires `git-tag` / `meta-tag` sources through the provider with single-scope / multi-scope guards; C.6 `G58CrossFamilyDepResolvabilityValidator` (scope-contains only; feed-probe reserved for Phase 2b) + 4 unit tests; C.7 G58 defense-in-depth — PreFlight mirror + Pack pre-gate sharing one `IG58CrossFamilyDepResolvabilityValidator` singleton, `PreflightReporter.ReportG58CrossFamilyResolvability` for per-check log UI; C.8 `PackageConsumerSmokeRunner` rejects empty `request.Versions` (runner is strict; `PackageConsumerSmokeTask.ShouldRun` skip-with-log on empty mirrors `PackageTask`); C.8a `Janset.Smoke.local.props` → `Janset.Local.props` file + path-service method + smoke-props import + `.gitignore` + JNSMK001 error text updated (docs reference sweep deferred to C.12). Test count 400 → 418 (+18); Windows build + test green, 689ms, 0 warnings; task graph flat via `--tree`. **Known state post-commit:** `smoke-witness.cs local` mode's `PackageConsumerSmoke` step now silent-skips (ShouldRun=false) until C.11 emits `versions.json` from `SetupLocalDev` + rewires both `local` / `ci-sim` modes to pass `--explicit-version`; IDE direct-restore of smoke csproj remains green via the renamed props auto-import. |
| **Slice C (closure: C.9–C.12)** | **DONE** | `15ae689` (closure) + `2c78184` (macOS CA1031/CA1515/CA2007 suppress on smoke-witness .NET 10.0.101) + `49bb30f` (path-service `ResolveVersionsOutputDirectory` + CleanArtifacts include, fixing a stale-versions.json read path for ci-sim runs starting on a dirty worktree) | C.9 `release.yml` grows `pack` (single Ubuntu runner, downloads consolidated-harvest + versions.json, expands JSON → `--explicit-version`, runs Package, uploads nupkg artifact) + `consumer-smoke` (7-RID matrix re-entry, downloads nupkgs + versions.json, runs PackageConsumerSmoke per-RID with `--rid` + `--explicit-version`) + `publish-staging` / `publish-public` stubs (`if: false` gated). C.10 test expansion: 2 `SetupLocalDevTaskRunner` versions.json tests, 2 `PackageConsumerSmokeTask` ShouldRun gate tests, 2 `PackageConsumerSmokeRunner` guard-rail tests (empty mapping → CakeException, null request → ArgumentNullException), 2 `G58CrossFamilyDepResolvabilityValidator` chain/concurrent tests. C.11 `SetupLocalDevTaskRunner` emits `artifacts/resolve-versions/versions.json` for Local profile; `smoke-witness.cs` rewired: `local` mode reads versions.json + passes `--explicit-version` to PackageConsumerSmoke, `ci-sim` mode gains PackageConsumerSmoke step with `--rid` + `--explicit-version`. C.12 PD-13 formally closed in ADR-003 §6; `Janset.Smoke.local.props` → `Janset.Local.props` reference sweep across canonical docs + `.gitignore`. **`smoke-witness --verbose / -v`**: `ParseArgs` replaces `ParseMode` (scans for `-v`/`--verbose` at any position; first non-flag arg is mode); non-verbose = streams drained + discarded, no console output, no log file; verbose = event-based `BeginOutputReadLine` / `BeginErrorReadLine` streaming live to `Console.Out` / `Console.Error` (raw — Cake ANSI sequences would be misinterpreted by Spectre) + full output captured to log file after exit; `WitnessContext` gains `bool Verbose`; header table shows on/off status. Test count 418 → 426 (+8). **Three-platform smoke witness (Windows + WSL + macOS Intel) all green** on `./tests/scripts/smoke-witness.cs local` + `ci-sim`. |
| **Slice E follow-up pass** (master-direct, P1-P8) | **DONE** | multi-commit (per-phase SHA trail in §2.2 status table) | All 8 phases closed via master-direct commits 2026-04-22 → 2026-04-25 from `bfc6713` baseline. P1-P4 absorbed prepare-native-assets-* discipline into `release.yml` matrix jobs (composites + cake-host build-and-test + Harvest+NativeSmoke inline + GCC11 + Kitware CMake 3.27 + sdl2-net retire + dotnet 8 SDK + win-x86 runtime Cake-owned + vcpkg-setup `runner.os` branch + `--versions-file` + P4e hygiene). buildTransitive layout: `b9f63f4` RuntimeIdentifier-aware (portable nested + RID-specific flat) + `357abe4` .NETFramework AnyCPU `_JansetSdlNet4xCopyRid` + smoke runner `-p:Platform`. P5 lock-files (4-commit sequence) narrowed strict mode to build host only after NU1004 (ILLink.Tasks runtime-patch drift) + NU1009 (NETFramework.ReferenceAssemblies Linux SDK auto-implicit collision); src/ stays on lenient mode for SDK-implicit-package drift absorption. P6 PublishStaging/Public Cake stubs (interface discipline; throws NotImplementedException). P7 three-platform witness: Windows + WSL Linux green local A-K (macOS deferred to CI per Deniz direction — no local Mac hardware); all 7 RIDs green via `release.yml` run 24932894291. P8 retire 5 YAMLs (`prepare-native-assets-*` family + `release-candidate-pipeline.yml`) + ADR-002 §2.3.1 delegate-hook amendment (Q17 closed) + CMakePresets 21→14 + CMakeUserPresets.json.example (Q18 closed) + Plan-A-geniş drift-audit sweep across knowledge-base / playbook / decisions / phases / onboarding / AGENTS.md / README.md (rewrites for ci-cd-packaging-and-release-plan.md + ci-troubleshooting.md archived to `docs/_archive/`). Test count 460/460. **Slice E follow-up pass closed; Phase 2b tail unlocked** (PA-2 witness, real `release.yml` hardening / first prerelease publication, `RemoteArtifactSourceResolver`, family/train orchestration). |
| **Slice E (partial: E1a-b + E2 infra)** | **PARTIAL — DONE** | `d9b3217` | **E1a** new `build-cake-host` job in `release.yml`: single-runner Ubuntu job that runs `dotnet restore` + `dotnet publish -c Release -p:UseAppHost=false` on `build/_build/Build.csproj` once per pipeline, emits a framework-dependent deployment (FDD) with `runtimes/*/native/` bundled (LibGit2Sharp per-OS `git2-*.{so,dll,dylib}` via Cake.Frosting.Git), uploads as `cake-host` artifact (`retention-days: 1`, run-scoped by design — per-run determinism over cross-run reuse). **E1b** all eight consumer jobs (`resolve-versions`, `preflight`, `generate-matrix`, `harvest`, `native-smoke`, `consolidate-harvest`, `pack`, `consumer-smoke`) gained `needs: build-cake-host` + `actions/download-artifact` of `cake-host`, and `dotnet run --project build/_build/Build.csproj -c Release --` became `dotnet ./cake-host/Build.dll` across every Cake invocation — no more per-job `dotnet restore/build` for the Cake host. `setup-dotnet` + `checkout` remain on every consumer: FDD needs the runtime and the Cake host reads repo-relative manifests (`manifest.json`, `CMakePresets.json`) from the working directory. **E2** Dockerfile + multi-arch GHCR workflow landed **but image NOT yet pushed to GHCR** — this is the mid-slice gate: `docker/linux-builder.Dockerfile` (ubuntu:20.04 focal base, glibc 2.31, LTS through 2030 — originally planned as debian:buster/glibc-2.28 following SkiaSharp pattern + manylinux_2_28 ecosystem baseline, corrected to focal on 2026-04-22 after first build surfaced buster's EoL archive-move: `deb.debian.org` no longer serves Release files for buster post-2024-03, so `apt-get update` 404s and `apt-get upgrade -y` is a no-op; focal restores live LTS security-patch surface; prepare-native-assets-linux.yml has been using ubuntu:20.04 already so apt package list ports directly. Verbatim port of `prepare-native-assets-linux.yml:37-80` apt preamble — package list, flags, autoconf-2.72-from-source SHA256 pin, order all preserved — plus two intentional additions: `cmake + ninja-build` for native-smoke and `git config --system --add safe.directory '*'` replacing prepare-*'s per-job config; `apt-get upgrade -y` added for monthly security-patch capture without changing focal's glibc major). `.github/workflows/build-linux-container.yml`: matrix build on `ubuntu-24.04` (amd64) + `ubuntu-24.04-arm` (arm64) via `docker buildx --push-by-digest`, merge job stitches digests into single OCI image index via `docker buildx imagetools create` emitting two tags (`focal-<yyyymmdd>-<sha>` immutable audit trail + `focal-latest` mutable pointer referenced by `manifest.json`). Triggers: `workflow_dispatch` (with `push: false` dry-run) + monthly cron (1st of each month 03:00 UTC) for upstream security-patch drift absorption. Retention: `actions/delete-package-versions@v5` keeps three most recent versions. **Not yet landed in this partial (deferred to Slice E follow-up pass):** E1c (absorb prepare-native-assets-*.yml into release.yml matrix jobs via composite actions for apt / brew platform prereqs + vcpkg-setup reuse, using Seçenek C inline conditional shape per Deniz direction — manifest `container_image` shared between build + consume, consumer-smoke inherits a richer-than-needed image but simpler axis), E1d (lock-file `packages.lock.json` + `<RestorePackagesWithLockFile>true</RestorePackagesWithLockFile>` + `--locked-mode` discipline), E3 (retire `prepare-native-assets-*.yml` family — only after E1c green on all three platforms), E4 (`PublishTask` stubs), E5 (final lint pass), E6 (broader canonical-doc sweep + §11 Q17 resolution), E7 (three-platform witness per A-K). |
| (end-of-pass chore) | **DONE** | `bfc6713` | `git merge --no-ff feat/adr003-impl` into `master`. 20 commits merged behind the merge parent; per-slice bisectability preserved. Pass re-scoped on Deniz direction 2026-04-22: this pass closes at Slice A→C (ADR-003 core implementation complete: three `IPackageVersionProvider` impls, stage-owned validation, flat task graph, runner-strict `--explicit-version`, G58 defense-in-depth, D-3seg versioning, package-first consumer contract, three-platform smoke witness green). Slice E is partially-landed as infrastructure (d9b3217) and its remainder (E1c/E1d/E3/E4-E7) moves to a separate follow-up pass starting from the `bfc6713` baseline. Rationale for the split: E2 custom GHCR image introduces a **manual mid-slice gate** (Dockerfile + build workflow land in code, then the operator runs `workflow_dispatch` to push the first GHCR image, then a subsequent commit updates `manifest.runtimes[].container_image` pointers) that does not fit a single-slice closure commit; E1c composite action design + E3 retire + E7 witness all benefit from having a real GHCR image to reference. No release tag push yet per §6.6 gating. |

### 2.2 Slice E follow-up pass (master post-merge, 2026-04-22+)

Scope deferred from pass-1 closure: E1c platform prereq absorption, E1d lock-file discipline, E3 prepare-*.yml retirement, E4 PublishTask stubs, E6 broader canonical-doc sweep + §11 Q17 resolution, E7 three-platform CI witness. Landed as master-direct commits (no dedicated branch per Deniz direction 2026-04-22 — pass-1 end-of-pass merge already consumed the long-branch cadence; follow-up work is CI infrastructure polish + witness, not ADR surface evolution).

**Design directions confirmed at P1 kick-off (2026-04-22):**

- `EnsureVcpkgDependencies` Cake target stays local / witness-only — CI uses `.github/actions/vcpkg-setup` which already runs `vcpkg install --triplet X` and populates `vcpkg_installed/<triplet>` directly (no Cake-side doubled work). The Cake target remains for `smoke-witness.cs ci-sim` + local-dev where GitHub composite actions don't run.
- NuGet cache is cross-platform by design: `.nupkg` restores are RID-agnostic; a single key family keyed on `hashFiles('**/packages.lock.json', '**/Directory.Packages.props', '**/*.csproj')` serves Windows / Linux / macOS runners. Path separator stays OS-native (`/` vs `\`) for operator-debuggability even though `actions/cache@v5` normalizes.
- Cake host tests MUST run in CI as part of `build-and-test-cake-host` (P2) with coverage collection + test report artifact + `Coverage-Check` Cake target gate — regression coverage is not a local-host-only surface.
- prepare-native-assets-*.yml retirement (E3) is the **last** phase — everything else must witness green across all three platforms first.

| Phase | Status | Commit | Scope |
| --- | --- | --- | --- |
| **P1** — Scaffolding | **DONE** | *filled at commit time* | `.github/actions/nuget-cache/action.yml` (cross-OS workspace-local `NUGET_PACKAGES` + `actions/cache@v5` platform-agnostic key; OS-aware path separators via `runner.os` conditional) + `.github/actions/platform-build-prereqs/action.yml` (macOS `brew install pkg-config autoconf automake libtool`; Linux + Windows no-ops — custom GHCR image + `IMsvcDevEnvironment` cover them respectively) + `Build.Tests.csproj` registered in `build/build.sln` under `_build` solution folder (sln presence is for IDE discovery; CI explicitly targets the test csproj, never the sln, to prevent future sln growth — extra test projects, samples — from dragging non-cake tests into the gate). Sanity: `dotnet test build/_build.Tests/Build.Tests.csproj -c Release` 426/426 green, 593ms on Windows host. §2.2 opened + §14 v2.13 kick-off entry. |
| **P2** — build-and-test-cake-host (E1a extension) | **DONE** | *filled at commit time* | `release.yml` `build-cake-host` display-name → "Build + Test Cake Host" (job ID unchanged so consumer `needs:` clauses stay intact). Steps: `nuget-cache` composite + **targeted** `dotnet restore build/_build.Tests/Build.Tests.csproj` (transitive covers Build.csproj via ProjectReference — never sln-wide) → **targeted** `dotnet test build/_build.Tests/Build.Tests.csproj -c Release --no-restore -- --coverage --coverage-output-format cobertura --coverage-output ${workspace}/artifacts/test-results/build-tests/coverage.cobertura.xml` → `dotnet publish build/_build/Build.csproj` FDD → `dotnet ./out/cake-host/Build.dll --target Coverage-Check` (ratchet gate) → 3 artifact uploads: `cake-host` FDD + `test-results` (TUnit HTML reports, `if: always()`) + `coverage` (cobertura xml, `if: always()` — both survive failures for debuggability). |
| **P3** — Harvest + NativeSmoke (E1c core) | **DONE** | *filled at commit time* | `release.yml` harvest + native-smoke job bodies absorbed: `actions/checkout@v6 { submodules: recursive, fetch-depth: 0 }` + `actions/setup-dotnet@v5` + `./.github/actions/platform-build-prereqs` + `./.github/actions/vcpkg-setup` (cache-key-base `${{ matrix.runner }}`, triplet `${{ matrix.triplet }}`, feature-flags `binarycaching`) + `./.github/actions/nuget-cache` + download `cake-host` artifact + `dotnet ./cake-host/Build.dll --target Harvest/NativeSmoke --rid X`. No Cake `EnsureVcpkgDependencies` step — vcpkg-setup composite already runs `vcpkg install` per triplet. |
| **P4** — ConsumerSmoke + Pack + other consumers (E1c tail) | **DONE** | *filled at commit time* | Every non-harvest consumer job (`resolve-versions`, `preflight`, `generate-matrix`, `consolidate-harvest`, `pack`, `consumer-smoke`) gains `./.github/actions/nuget-cache` for uniform env + cross-run cache. `consumer-smoke` also gains `./.github/actions/platform-build-prereqs` (macOS brew at runtime just-in-case; Linux + Windows no-op). No vcpkg-setup on these — consumer-side Cake host only does NuGet restore / pack / test, never vcpkg invocation. `pack` + `consumer-smoke` originally used `mapfile -t args < <(jq ...)` expansion from versions.json artifact — **subsequently replaced in P4c** with `--versions-file` CLI option for cross-platform compatibility. |
| **P4b** — CI iteration fixes (post first end-to-end run) | **DONE** | `c3e230c` + `907ec86` + `7fa724b` + `eecc921` | Four consecutive fixes land after P1–P4 surfaced real CI issues. (1) `c3e230c`: tracked `build/coverage-baseline.json` (shadow-ignored by the coverlet glob) + re-ratcheted to 65.0 / 58.0 floors + TUnit HTML Option A auto-upload via `actions/github-script` (`ACTIONS_RUNTIME_TOKEN` + `ACTIONS_RESULTS_URL`). (2) `907ec86`: `actions/github-script@v7` → `@v9` (Node 24) + macOS brew install idempotent per-formula (kills "already installed" noise). (3) `7fa724b`: GCC 11 in builder image via `ubuntu-toolchain-r/test` PPA (focal's 9.4 fails libyuv arm64 `i8mm` + libwebp x64 `_mm256_cvtsi256_si32` inlining bug #98495); update-alternatives split so `cc` / `c++` stay as master alternatives; `ENV CC=/usr/bin/gcc-11 CXX=/usr/bin/g++-11` belt-and-suspenders. (4) `eecc921`: CMake via Kitware APT (3.27+) to support schema-v3 presets (focal 3.16.3 silently misinterpreted `cmake --preset linux-x64` as a source dir) + `release.yml` Pack + ConsumerSmoke concrete-family filter (sdl2-net placeholder) + `CMakePresets.json` expanded to all 7 RIDs × 3 variants (release/debug/interactive) + new `Domain/Runtime/MsvcTargetArch.cs` enum with RID→arch mapping (fail-fast on unknown RID) + `IMsvcDevEnvironment.ResolveAsync(MsvcTargetArch, CT)` per-target-arch cache for x64/x86/arm64 cross-compile vcvarsall args + `NativeSmokeTaskRunner.ApplyMsvcEnvironmentAsync(settings, rid)` threads rid through. Test count 426 → 446 (+20 `MsvcTargetArchTests`). |
| **P4c** — CI run fixes (runs 24786888297 → 24797959687 → 24800741006) | **DONE** | `b0ccda0` + `bc652d1` + `3fe0303` | Three CI runs drove three commits resolving all P4b-era regressions. **Run 24786888297** (post-`eecc921`): Harvest 7/7 ✓, NativeSmoke 6/7 ✓ (win-arm64 cross-compile runs but can't execute ARM64 binary on x64 runner — expected, not a build failure), ConsumerSmoke 0/7 ✗ (three root causes identified below). **Commit `b0ccda0`**: (1) `release.yml` replaced `jq`/`mapfile` version-extraction with `--versions-file artifacts/resolve-versions/versions.json` in PreFlight, Pack, and ConsumerSmoke jobs — eliminates bash-only shell constructs that failed on Windows runners and simplifies the version-passing contract; (2) collapsed standalone `native-smoke` job into the `harvest` job (NativeSmoke runs inline after Harvest within each RID matrix entry — one fewer job, same coverage, NativeSmoke result is per-RID not cross-RID); (3) added Cake `Info` diagnostic step to `harvest` and `consumer-smoke` matrix jobs for debuggability; (4) `manifest.json` `win-arm64` runner corrected to `windows-11-arm` (from previous session). **Commit `bc652d1`**: removed `sdl2-net` placeholder from `manifest.json` (`package_families` + `library_manifests`) — root cause of Pack failure: `ResolveVersions` emitted all families including the null-project placeholder into `versions.json`, downstream Pack correctly rejected it per ADR-003 §2.2 ("scope = versions.keys — if you don't want to pack a family, don't put it in the mapping"); `SetupLocalDev` (local path) already filtered to concrete families, so local never hit this. Characterization tests updated (families 6→5, libraries ≥6→≥5). `plan.md` updated: sdl2-net marked Phase 3 / #58 with full re-add checklist. **Run 24797959687**: Harvest+NativeSmoke 7/7 ✓, Pack ✓, ConsumerSmoke partial — `linux-arm64` failed on missing .NET 8 runtime. **Commit `3fe0303`**: added `dotnet-version: 8.0.x` to ConsumerSmoke's `setup-dotnet` step — smoke project targets `$(ExecutableTargetFrameworks) = net9.0;net8.0;net462`, but `global-json-file` only installs .NET 9 SDK; net462 already handled per-platform by `ShouldSkipTfm` (native Windows, skip Linux, Mono-probe macOS). **Run 24800741006**: Harvest 7/7 ✓, Pack ✓, ConsumerSmoke progressing (linux-arm64 net8.0 now runs; remaining failures under investigation). Test count stable at 455/455. |
| **P4d** — TFM SDK automation research | not started | — | **Research item:** Currently the consumer-smoke job hardcodes `dotnet-version: 8.0.x` alongside `global-json-file` in YAML. This duplicates the TFM policy defined in `Directory.Build.props` (`$(ExecutableTargetFrameworks)`). Investigate whether Cake can automate SDK installation requirements: (1) Can `PackageConsumerSmokeRunner` read `$(ExecutableTargetFrameworks)` from the smoke csproj's MSBuild evaluation and emit a required-SDKs list? (2) Should `generate-matrix` output include a `dotnet-versions` array derived from manifest or Directory.Build.props, consumed by `setup-dotnet`? (3) Alternative: a `setup-dotnet-for-tfms` composite action that reads TFMs from a project file and installs all required SDKs. Goal: single source of truth for TFM policy (Directory.Build.props) without YAML duplication. Low priority — current YAML hardcode works and is explicit. |
| **P5** — Lock-file discipline (E1d) | **DONE** | `b3a66a4` + `734e69a` + `a4f1cd0` + `c4bfd73` | Four-commit sequence. `b3a66a4` introduced `<RestorePackagesWithLockFile>true</...>` in `Build.csproj` + `Build.Tests.csproj` + `src/Directory.Build.props` (10 src/* csprojs inherit) with `<RestoreLockedMode Condition="'$(GITHUB_ACTIONS)' == 'true'">true</...>` (CI strict, local lenient — no `release.yml` edit per YAML-thin invariant). 12 in-scope `packages.lock.json` files generated and committed. CI run 24931815856 caught NU1004 on `Microsoft.NETFramework.ReferenceAssemblies` (Windows-vs-Linux SDK-implicit asymmetry — Windows ships local v4.6.2 ref assemblies, Linux/macOS auto-pull the package); `734e69a` pinned it explicitly in `Directory.Packages.props` + conditional PackageReference in `src/Directory.Build.props`. CI run 24932084975 then caught NU1004 on `Microsoft.NET.ILLink.Tasks` (runtime-patch asymmetry — package ships with each .NET runtime patch, local SDK 9.0.309 / 8.0.x and CI Linux runner SDK drift past each other every Microsoft cadence); `a4f1cd0` narrowed `<RestoreLockedMode>` to build host only (removed from `src/Directory.Build.props`) — strict mode survives where the package surface is bounded and not patch-driven; library projects keep lock files for PR-diff visibility but skip strict mode. CI run 24932434888 then surfaced NU1009 on Linux/macOS — the previous `Microsoft.NETFramework.ReferenceAssemblies` pin collided with Linux SDK's implicit-reference marker for that package on every csproj (CPM is root-level → every csproj sees the version → NU1009 even on consumer csprojs with no PackageReference); `c4bfd73` reverted the pin (symmetric to `734e69a`) — with src/ on lenient mode the original cross-platform NU1004 is now absorbed by lock-file regeneration on the failing platform. Final CI run 24932894291 all green. |
| **P6** — PublishTask stubs (E4) | **DONE** | `d190b5b` | `Application/Publishing/PublishTaskRunner.cs` (concrete; no interface — single implementation, single caller surface, AGENTS.md interface discipline) throws `NotImplementedException` with Phase-2b pointer. `Tasks/Publishing/PublishStagingTask.cs` + `PublishPublicTask.cs` (golden thin adapters delegating to runner). `Program.cs` registers `PublishTaskRunner` as singleton; `ProgramCompositionRootTests` Hybrid composition test gains a resolution assertion. Folder named `Tasks/Publishing/` (not `Tasks/Publish/`) for namespace consistency with `Application/Publishing/` and to avoid `.gitignore` line 188 `publish/` rule clash. `release.yml` unchanged: `publish-staging` / `publish-public` stay gated `if: false`. `[SuppressMessage MA0025]` on `RunAsync` — `NotImplementedException` is semantically correct (planned, not unsupported). Verified locally: 460/460 tests PASS; `--target PublishStaging \| PublishPublic` discoverable via Cake `--tree`, both throw with Phase-2b message. |
| **P7** — Three-platform witness (E7) | **DONE** (Windows + WSL Linux; macOS deferred — no hardware) | `2ea5c30` (playbook gotcha note) | A-K formal walkthrough at master `d190b5b` on two platforms — macOS deferred per Deniz direction (no Mac hardware available locally; CI dispatch covers macOS via release.yml runs). **Windows (win-x64)**: A 460/460 ✓ — B 0 errors ✓ — C 20 targets in `--tree` incl. `PublishStaging`/`PublishPublic` ✓ — D PreFlight green ✓ — D1 vcpkg warm 3.4ms install ✓ — E Harvest 5 lib in 3.09s, 1 primary 0 runtime per lib ✓ — F ConsolidateHarvest ✓ — F1 Inspect-Deps hybrid-static, no leaked codecs ✓ — G NativeSmoke 29/29 PASS in 9.3s ✓ — J 15 nupkg (5 family × 3) in 41.8s via SetupLocalDev composition ✓ — K PackageConsumerSmoke net9 12 + net8 12 + net462 11 = **35/35 PASS** in 39.5s ✓. **WSL Linux (linux-x64)**: same A-K, 5-lib symlink chains preserved, 29/29 NativeSmoke incl. MIDI decoder fonctional via freepats, K 12+12=24 PASS (net462 correctly skipped per Mono+TUnit incompat). **Gotcha caught + documented**: WSL `appendWindowsPath=true` default puts `/mnt/c/Program Files/dotnet` ahead of `~/.dotnet` → Cake host starts on Linux dotnet but child `dotnet pack` picks Windows dotnet via naked PATH lookup → MSBuild fails MSB1001 on Linux paths. D-G unaffected (Cake's `IPathService` resolution); J/K need Linux-only PATH override (`export PATH=$HOME/.dotnet:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin`). Documented in `cross-platform-smoke-validation.md` §"WSL / Linux" via `2ea5c30`. |
| **P8** — Retirement + doc sweep tail (E3 + E6) | **DONE** | `3c0bb86` (retirement) + `d84f83c` (playbook AnyCPU) + closure commit | **P8.1** `3c0bb86` deleted `.github/workflows/prepare-native-assets-{linux,macos,windows,main}.yml` + `release-candidate-pipeline.yml` (5 files, 637 deletions); release.yml + build-linux-container.yml are the only retained workflows. **P8.2** `d84f83c` added `cross-platform-smoke-validation.md` §".NETFramework AnyCPU Consumer Guideline" — resolution priority table (Platform → Prefer32Bit → OSArchitecture, RuntimeIdentifier deliberately ignored due to SDK auto-x86 indistinguishability), three opt-in patterns for win-x86 native shipping, smoke runner `AppendNet4xPlatformArgument` mention, Common.targets in-file comments declared canonical reference. **P8.3 (this commit)**: ADR-002 gains §2.3.1 amendment formalizing delegate-hook test pattern for non-mockable third-party boundaries; Q17 closed. **P8.4 (this commit)**: `tests/smoke-tests/native-smoke/CMakePresets.json` reduced 21→14 buildPresets (drop `*-interactive` variants for all 7 RIDs); `CMakeUserPresets.json.example` template ships interactive entries for every RID; `README.md` "Two Modes" + "Interactive Mode" sections rewritten around UserPresets opt-in flow; `.gitignore` excludes `tests/smoke-tests/native-smoke/CMakeUserPresets.json` (template stays VCS-tracked); Q18 closed. **Slice E follow-up pass closure**: this commit. |
...
- **Q17 — Cake.Frosting.Git addin testability: delegate hook vs integration tests vs consolidated seam** *(opened 2026-04-21 during Slice C.3 implementation; must be addressed before the Slice E broader doc-alignment sweep)*. **Context:** Cake.Frosting.Git is LibGit2Sharp-backed and bypasses `ICakeContext.FileSystem` — it hits `System.IO.Directory.Exists` via LibGit2Sharp's native binary directly. Unit tests built on `FakeFileSystem` (repo fixture `FakeRepoBuilder` with `C:/repo` virtual path) fail with `RepositoryNotFoundException` when the runner calls a Cake.Git alias. **Slice C.3 decision:** introduced a test-only delegate hook on `PackageTaskRunner` ctor (optional `Func<ICakeContext, DirectoryPath, string>? resolveHeadCommitSha = null` param defaulting to `DefaultResolveHeadCommitSha` which wraps `GitLogTip` + null guard); tests pass a stub `(_, _) => "0123...01234567"` lambda, production path is unchanged via the default. **Accepted as conscious tech-debt** to unblock C.3 without inventing an `IGitCommandRunner` interface (ADR-002 §2.3 three-criteria rule: no second impl today, no independent axis of change, test-mock alone insufficient justification) and without the fixture complexity of `Repository.Init()`-in-temp-dir integration tests for a single-call HEAD SHA resolver. **Slice C.4 direction:** `GitTagVersionProvider` carries richer behaviour (`GitDescribe`, tag discovery, topological sort across multi-family scope) — will use **integration tests with real ephemeral git repo in `Path.GetTempPath()`** (production resolver path exercised directly, no delegate hook). The delegate-hook pattern therefore does not propagate beyond `PackageTaskRunner.ResolveHeadCommitSha`. **Open sub-questions for Slice E pre-doc-alignment review:**
  - Does the delegate hook stay at `PackageTaskRunner` long-term, or does it consolidate with any other Cake.Frosting.Git usage we accumulate between C.3 and Slice E? If the C.4 integration-test pattern proves ergonomic, we might retire the C.3 delegate hook in favor of a temp-repo fixture (two callers then: provider tests + runner tests both ride the fixture). If the delegate hook ergonomics beat the fixture cost, maybe C.4 also ends up with a hook (provider gains a mockable seam at the alias boundary).
  - Post-C.4, should we capture a narrow test-only extension under `Build.Tests/Fixtures/` that wraps `Repository.Init()` + a deterministic initial commit, callable from any test that wants a real git repo? Cuts setup boilerplate if ≥2 tests need it; premature if stays at 1.
  - ADR-002 §2.3 stance toward "delegate-only" test hooks: the ADR is strict about *interfaces*, silent on delegates. Either we extend §2.3 to cover delegate hooks with the same criteria (treating them as "lightweight interface" and requiring the same justification), or we explicitly document that delegate hooks with a default-to-production-behaviour fallback are a permitted testability pattern for non-mockable third-party boundaries. The second reading is what C.3 relies on; needs explicit ADR-002 note or a sibling ADR if promoted. Review + sign-off during Slice E canonical-doc sweep pass.
- **Q18 — CMakePresets.json developer experience: split user-local presets + retire `-interactive` preset variants** *(opened 2026-04-22 during Slice E follow-up pass P3 / CMakePresets expansion)*. **Context:** `tests/smoke-tests/native-smoke/CMakePresets.json` currently carries three variants per RID (`<rid>`, `<rid>-debug`, `<rid>-interactive` where interactive = debug + `SMOKE_INTERACTIVE=ON` cache var), yielding 21 → 28 presets after the PA-2 expansion. Deniz flagged the "interactive" variant as possibly-non-canonical and asked for a research pass on CMakePresets best practices. **Research** ([martin-fieber.de TL;DR](https://martin-fieber.de/blog/cmake-presets/), [Matt Gibson — Simplify C++ dev env](https://mgibson.ca/posts/simplify-your-c-development-environment-using-cmakepresets-json/), [official `cmake-presets(7)`](https://cmake.org/cmake/help/latest/manual/cmake-presets.7.html), [VS CMake Presets](https://learn.microsoft.com/en-us/cpp/build/cmake-presets-vs), [CLion CMake Presets](https://www.jetbrains.com/help/clion/cmake-presets.html)) resolved: the ecosystem splits `CMakePresets.json` (project-wide, VCS-tracked, Debug/Release canonical axes) from `CMakeUserPresets.json` (developer-local, VCS-ignored, custom overrides). Feature flags like `SMOKE_INTERACTIVE` are idiomatically either `-D` cache-variable overrides on the command line, or developer-per-user UserPresets entries — not project-wide preset triplets. CLion (presets v2–10) and VS 2022 17.5+ (schema v5) both auto-detect UserPresets alongside project presets. **Direction selected** (2026-04-22, pending implementation): (a) drop the `*-interactive` preset variants from `CMakePresets.json` (21 → 14 presets across 7 RIDs × 2 variants release/debug); (b) add `tests/smoke-tests/native-smoke/CMakeUserPresets.json.example` as a copyable template demonstrating an interactive preset inheriting `<rid>-debug` with `SMOKE_INTERACTIVE=ON` added; (c) add a `tests/smoke-tests/native-smoke/README.md` sub-doc covering "CLion / VS / CLI usage + how to opt into interactive via UserPresets or `-DSMOKE_INTERACTIVE=ON`"; (d) `.gitignore` entry `tests/smoke-tests/native-smoke/CMakeUserPresets.json` (keep the `.example` tracked). **Scope landing:** Slice E follow-up pass P8 (alongside the broader doc sweep) or a standalone follow-up commit post-pass if P8 churn is too large to bundle. Not blocking P5 (lock-file) / P6 (Publish stubs) / P7 (three-platform witness).

### Previously open, now closed

- **Q17 (Cake.Frosting.Git delegate-hook stance):** closed 2026-04-25 — ADR-002 gains a new §2.3.1 amendment formalizing the delegate-hook pattern as a permitted testability mechanism for non-mockable third-party boundaries (Cake.Frosting.Git/LibGit2Sharp bypassing `ICakeContext.FileSystem`). Four invariants codified: default delegate exercises production path; optional ctor-injected only; bounded to the specific call-site; documented inline. Counter-rule: ≥3 callsites or clustering promotes to a real interface or `Build.Tests/Fixtures/` extension. `PackageTaskRunner.ResolveHeadCommitSha` stays as the single permitted call-site; `GitTagVersionProvider` continues to use the integration-test fixture pattern (no delegate hook).
- **Q18 (CMakePresets developer-experience refactor):** closed 2026-04-25 — `tests/smoke-tests/native-smoke/CMakePresets.json` reduced from 21 to 14 buildPresets by dropping `*-interactive` variants across all 7 RIDs (Release + Debug remain canonical). Interactive variants moved into a developer-local `CMakeUserPresets.json` opt-in pattern aligned with the official `cmake-presets(7)` recommendation; `CMakeUserPresets.json.example` template ships interactive entries for every RID, `tests/smoke-tests/native-smoke/CMakeUserPresets.json` added to `.gitignore`. README "Two Modes" + "Interactive Mode" sections rewritten to point at the UserPresets opt-in flow (`cp CMakeUserPresets.json.example CMakeUserPresets.json`) plus the `-DSMOKE_INTERACTIVE=ON` CLI alternative.
...
| 2026-04-22 | v2.10 — Slice C partial landing (C.1–C.8a). Mid-slice commit on `feat/adr003-impl` per Deniz direction (2026-04-22): "C.8 serisi de bitsin, sonra conventional commit." Eight of thirteen Slice C sub-steps land in one batched commit; C.9–C.12 remain open and move to next session via `.github/prompts/s10-adr003-slice-c-continuation.prompt.md`. Design trail captured during implementation: (a) §11 Q17 opened when Cake.Frosting.Git's `ICakeContext.FileSystem` bypass collided with `FakeFileSystem`-backed unit tests — delegate-hook pattern chosen for `PackageTaskRunner.ResolveHeadCommitSha` over an `IGitCommandRunner` interface (ADR-002 §2.3 three-criteria rule) or `Repository.Init()`-in-temp-dir fixture integration for a single-call HEAD resolver; pattern bounded to this one call-site by contract — `GitTagVersionProvider` uses the integration-test path instead; (b) `Targeted`/`Train` naming for `GitTagScope` sum-type preferred over `Single`/`Multi` (CA1720 `Single` conflicts with `System.Single`; `Targeted`/`Train` maps 1:1 to the release-vocabulary in `docs/knowledge-base/release-lifecycle-direction.md`); (c) G58 validator emits the same `G58CrossFamilyValidation` shape into both PreFlight (reporter-logged, pre-harvest fail-fast) and Pack (runner-logged, pre-nupkg gate) — one implementation, two call sites; (d) `Janset.Smoke.local.props` → `Janset.Local.props` broadens the file's scope from smoke-only to repo-wide local-feed override so future samples / AST-test / sandbox trees ride the same IDE direct-restore mechanism; (e) runner-strict `--explicit-version` on `PackageConsumerSmokeRunner` retired the parallel "trust local.props" fallback — single-source-of-truth for Cake invocations, with the props file reduced to a dev-ergonomics artefact Cake never consumes. **Witness delta:** `smoke-witness.cs local` silent-skips `PackageConsumerSmoke` until C.11; `ci-sim` unchanged (still skipping); all other witness paths unaffected. **Test count 400 → 418** (+18) Windows, 689ms; LayerDependencyTests green; `--tree` flat. | Deniz İrgin + session collaboration |
| 2026-04-22 | v2.11 — Slice C closure (C.9–C.12). Remaining five sub-steps close out Slice C: C.9 `release.yml` grows `pack` job (single Ubuntu, consolidated-harvest + versions.json → `--explicit-version` expansion → Package → nupkg upload) + `consumer-smoke` job (7-RID matrix, nupkgs + versions.json → per-RID `PackageConsumerSmoke --rid --explicit-version`) + `publish-staging` / `publish-public` stubs (`if: false`). C.10 eight new tests: `SetupLocalDevTaskRunner` versions.json emission (Local writes, RemoteInternal skips); `PackageConsumerSmokeTask` ShouldRun gate (empty→false, present→true); `PackageConsumerSmokeRunner` guards (empty→CakeException, null→ArgumentNullException); `G58CrossFamilyDepResolvabilityValidator` chain-DependsOn (3-family all-InScope) + concurrent-missing (2 satellites missing core → 2 Missing checks). C.11 `SetupLocalDevTaskRunner` emits `versions.json` for Local profile via `WriteVersionsJsonAsync`; `smoke-witness.cs` rewired — `local` reads versions.json + passes `--explicit-version` to PackageConsumerSmoke, `ci-sim` gains PackageConsumerSmoke step with `--rid` + `--explicit-version`; `PrintConsumerSmokeSkipNote` removed. C.12 PD-13 formally closed in ADR-003 §6; `Janset.Smoke.local.props` → `Janset.Local.props` reference sweep across `.gitignore` + canonical docs. **`smoke-witness --verbose / -v`**: non-verbose = drain + discard (no log, no console); verbose = live `BeginOutputReadLine` / `BeginErrorReadLine` streaming to `Console.Out` / `Console.Error` + log file written after exit; `WitnessContext.Verbose` field; header table status row. Test count 418 → 426 (+8). Build green, 426/426 tests pass. | Deniz İrgin + session collaboration |
| 2026-04-22 | v2.12 — Pass closure + Slice E pass-split. **Three-platform smoke witness green** (Windows + WSL + macOS Intel) on `./tests/scripts/smoke-witness.cs local` + `ci-sim` — validates Slice C closure on every platform the current pass targets. **Slice E partial (E1a-b + E2 infra)** landed as `d9b3217`: (a) `release.yml` gains a single `build-cake-host` job that does `dotnet publish -c Release -p:UseAppHost=false` and uploads the FDD as a `cake-host` artifact, and all eight consumer jobs rewired to `dotnet ./cake-host/Build.dll` (skipping per-job `dotnet restore/build` for the Cake host entirely); (b) `docker/linux-builder.Dockerfile` (ubuntu:20.04 focal base, glibc 2.31, LTS through 2030 — originally planned as debian:buster/glibc-2.28 per SkiaSharp pattern + manylinux_2_28; corrected to focal on 2026-04-22 after first build revealed buster EoL archive-move broke `apt-get update` + rendered `apt-get upgrade -y` no-op, focal restores live LTS security-patch surface and matches prepare-native-assets-linux.yml's base exactly; verbatim port of `prepare-native-assets-linux.yml:37-80` apt preamble plus two intentional additions `cmake+ninja-build` for native-smoke and image-level `git config --system --add safe.directory '*'` replacing per-job config) + `.github/workflows/build-linux-container.yml` (multi-arch native matrix amd64+arm64 → `docker buildx imagetools create` → GHCR `focal-<yyyymmdd>-<sha>` + `focal-latest` tags, monthly cron + `workflow_dispatch` with dry-run input, retention via `actions/delete-package-versions@v5` keeps 3 versions). **Pass re-scoped to Slice A → C closure** per Deniz direction 2026-04-22: ADR-003 core implementation surface (three `IPackageVersionProvider` impls, stage-owned validation, flat task graph, runner-strict `--explicit-version`, G58 defense-in-depth, D-3seg versioning, package-first consumer contract, Windows+WSL+macOS Intel witnesses green) is the natural closure point for the pass opened on 2026-04-20. Slice E remainder (E1c composite action + apt/brew prereqs absorption, E1d lock-file discipline, E3 retire `prepare-native-assets-*.yml`, E4 PublishTask stubs, E5 lint, E6 broader doc sweep + §11 Q17 resolution, E7 three-platform witness per A-K checkpoints) moves to a **separate follow-up pass** starting from the `bfc6713` master baseline. Rationale for the split: E2 custom GHCR image introduces a **manual mid-slice gate** (Dockerfile + build workflow land in code, operator runs `workflow_dispatch` to push first GHCR image, subsequent commit updates `manifest.runtimes[].container_image` pointers) that doesn't fit a single-slice closure commit; E1c composite action design + E3 retire + E7 witness all benefit from having a real GHCR image to reference. **End-of-pass merge** `bfc6713 Merge branch 'feat/adr003-impl'` on master at 2026-04-22 via `git merge --no-ff` (per-slice bisectability preserved, 20 commits behind merge parent). No release tag push per §6.6 gating (still blocked on Slice E follow-up pass closure). §11 Q17 (Cake.Frosting.Git delegate hook vs `TempGitRepo` fixture ADR-002 §2.3 stance) carries into the follow-up pass as the sole unresolved design debt from Slice C. | Deniz İrgin + session collaboration |
| 2026-04-22 | v2.14 — Slice E follow-up pass CI iteration (P4b + P4c observations). After P1–P4 landed, four consecutive fix commits took the pipeline from "Harvest fail on vcpkg precondition" to "6/7 NativeSmoke + Pack green, ConsumerSmoke 0/7, NativeSmoke win-arm64 fail at cross-compile execution step". (a) `c3e230c` tracked the shadow-ignored `coverage-baseline.json` + re-ratcheted to 65/58 floors + TUnit auto-upload Option A; (b) `907ec86` bumped `github-script` to v9 + made brew install idempotent; (c) `7fa724b` installed GCC 11 in the builder image (Kitware-independent, via `ubuntu-toolchain-r/test` PPA) to get past libyuv arm64 `i8mm` + libwebp x64 `_mm256_cvtsi256_si32` intrinsic-inlining bugs, with `update-alternatives` split across `gcc`/`g++`/`gcov` (slave) + `cc` + `c++` (separate masters) + `ENV CC/CXX` belt-and-suspenders; (d) `eecc921` landed four logically-grouped fixes: Kitware APT for CMake 3.27+ (focal 3.16.3 lacks preset schema v3, silently treated `--preset linux-x64` as a source dir), `release.yml` Pack + ConsumerSmoke concrete-family filter (sdl2-net placeholder guard), `CMakePresets.json` expansion to all 7 RIDs × 3 variants, and a brand-new `Domain/Runtime/MsvcTargetArch.cs` enum + RID→arch `FromRid` mapping + `ToVcvarsArg` host+target cross-compile combo builder; `IMsvcDevEnvironment.ResolveAsync(MsvcTargetArch, CT)` per-target-arch `ConcurrentDictionary` cache; `NativeSmokeTaskRunner.ApplyMsvcEnvironmentAsync(settings, rid)` threads rid through. Test count 426 → 446 (+20 new `MsvcTargetArchTests` covering RID mapping happy-paths, unknown-RID fail-fast, null/whitespace guard, full host × target cross-compile matrix, unsupported-host guard). **Remaining gates** for the follow-up pass carry into the next session: (1) NativeSmoke win-arm64 cross-compile binary execution — runner is Windows x64, compiled ARM64 .exe cannot run there; options are native `windows-11-arm` runner, QEMU, or a Cake-level `if (host == target) run else build-only` skip; (2) ConsumerSmoke 0/7 — investigation pending, likely feed path / package restore / TUnit assertion surface; (3) Harvest + NativeSmoke CI-layer merge — Deniz direction 2026-04-22 "NativeSmoke is Harvest's guardrail; any RID where NativeSmoke fails is a failed Harvest" — collapse the two matrix jobs into one (vcpkg-setup once per RID, harvest output uploaded after NativeSmoke passes, ConsolidateHarvest semantics unchanged). | Deniz İrgin + session collaboration |
| 2026-04-23 | v2.16 — P4d resolution + cache-identity cleanup (working tree). Two follow-up investigations closed. (a) `ConsumerSmoke (win-x86)` no longer relies on an inline `release.yml` PowerShell branch: new `Domain/Runtime/IDotNetRuntimeEnvironment.cs` + `Infrastructure/DotNet/DotNetRuntimeEnvironment.cs` derive required x86 runtime channels from the smoke project's executable TFMs, bootstrap them via the official `dotnet-install.ps1` into a temp cache, and inject `DOTNET_ROOT_X86` + `DOTNET_ROOT(x86)` only into child `dotnet test` invocations inside `PackageConsumerSmokeRunner`; the Cake host and workflow-level `setup-dotnet` stay on x64. This mirrors the existing `IMsvcDevEnvironment` ownership pattern: policy in Cake, YAML thin. (b) `vcpkg-setup` no longer branches on the stale literal `inputs.runner-os == 'windows-latest'`; it now keys on `runner.os`, which fixes the accidental Git-Bash-on-Windows path in `release.yml` harvest jobs once manifest runner labels moved to `windows-2025` / `windows-11-arm`. The same composite action now resolves mutable `container_image` tags (for example `ghcr.io/...:focal-latest`) to immutable per-platform child manifest digests inline for its cache key by matching the registry's manifest-list JSON against `runner.os` + `runner.arch`, with a top-level digest fallback when no child match exists; `release.yml` therefore keeps the convenience tag for job execution without paying for a dedicated pre-matrix helper runner. Broad restore-key fallback trimmed to the overlay-hash-specific form only. Test count **455 → 460** (+5): new `DotNetRuntimeEnvironmentTests` cover runtime-channel derivation, non-win-x86 no-op, non-Windows host guard, and DI contract; full `dotnet test build/_build.Tests/Build.Tests.csproj -c Release` green on Windows host. | Deniz İrgin + session collaboration |
| 2026-04-22 / 2026-04-23 | v2.15 — P4c resolution (3 commits) + P4d TFM research item opened. **Three CI runs** drove three commits that resolved all ConsumerSmoke + Pack failures from P4b-era regressions: (a) `b0ccda0` replaced `jq`/`mapfile` version-extraction with Cake-native `--versions-file` in PreFlight/Pack/ConsumerSmoke jobs (cross-platform: jq unavailable on Windows runners, mapfile is bash-only); collapsed `native-smoke` job into `harvest` job (NativeSmoke runs inline after Harvest per-RID — Deniz direction "NativeSmoke is Harvest's guardrail"); added Cake `Info` diagnostic step to harvest + consumer-smoke matrix jobs; updated release.yml header comments for new 10-job graph. (b) `bc652d1` removed `sdl2-net` placeholder from `manifest.json` — root cause of Pack failure: `ResolveVersions` → `ManifestVersionProvider` emitted ALL families (including sdl2-net with `managed_project: null`) into versions.json when scope was empty, but downstream Pack correctly rejected the placeholder per ADR-003 §2.2 ("scope = versions.keys"); `SetupLocalDev` already filtered to concrete families (line 98-114 of `SetupLocalDevTaskRunner`), so local path was never affected. Characterization tests updated (families 6→5, library_manifests ≥6→≥5); `plan.md` sdl2-net marked as Phase 3 / #58 with full re-add checklist (managed csproj + .Native csproj + overlay port + manifest entries + vcpkg features + harvest validation). (c) `3fe0303` added `dotnet-version: 8.0.x` to ConsumerSmoke's `setup-dotnet` — smoke project targets `$(ExecutableTargetFrameworks) = net9.0;net8.0;net462` but global.json only installs .NET 9 SDK; net462 already handled per-platform by `PackageConsumerSmokeRunner.ShouldSkipTfm` (native on Windows, always-skip on Linux, Mono-probe on macOS). Run 24800741006 showed Harvest 7/7 ✓, Pack ✓, ConsumerSmoke progressing with linux-arm64 past the net8.0 barrier. Test count stable at 455/455. **P4d research item opened:** currently `dotnet-version: 8.0.x` is hardcoded in YAML alongside `global-json-file`, duplicating the TFM policy in `Directory.Build.props`. Research needed: should Cake (or `generate-matrix`) derive required SDK versions from `$(ExecutableTargetFrameworks)` and emit them into the matrix JSON so `setup-dotnet` pulls from a single source of truth? Three options sketched (ManifestVersionProvider-level, generate-matrix output, composite action). Low priority — current YAML hardcode is explicit and works. | Deniz İrgin + session collaboration |
| 2026-04-25 | v2.17 — Slice E follow-up pass closure + doc-sweep audit (Plan A geniş per Deniz direction). All 8 phases (P1-P8) closed via master-direct commits 2026-04-22 → 2026-04-25 from `bfc6713` baseline; per-phase SHA trail in §2.2. **Q17 closed** via ADR-002 §2.3.1 amendment formalizing the delegate-hook test pattern for non-mockable third-party boundaries (Cake.Frosting.Git/LibGit2Sharp bypassing `ICakeContext.FileSystem`); 4 invariants codified, counter-rule defined. **Q18 closed** via CMakePresets refactor (21→14 buildPresets; `CMakeUserPresets.json.example` template ships interactive entries for every RID; `.gitignore` excludes the user file; `tests/smoke-tests/native-smoke/README.md` rewritten around UserPresets opt-in flow). **P5 lock-file discipline**: 4-commit sequence (initial → NETFramework.ReferenceAssemblies pin → narrow strict mode to build host only → revert NETFramework.ReferenceAssemblies pin) settled on lenient mode in `src/` (absorbs SDK-implicit-package drift on monthly runtime patches) + strict mode on `build/_build*` (Cake host determinism). **P7 three-platform witness**: Windows + WSL Linux green local A-K at master `d190b5b`; macOS deferred to CI per Deniz direction. All 7 RIDs green via `release.yml` run 24932894291. **P8 doc-sweep audit (Plan A geniş)**: post-Cake-+-CI-hatm comparison of code-current state vs each canonical doc; obsolete/superseded docs (`docs/_archive/`, `docs/research/`, `docs/reviews/`, retire-to-stub phase docs) skipped per Deniz direction. Targeted edits across plan.md, phase-2-adaptation-plan.md, cake-build-architecture.md, release-lifecycle-direction.md, release-guardrails.md, onboarding.md, AGENTS.md, README.md, cross-platform-smoke-validation.md, local-development.md, unix-smoke-runbook.md. Substantial rewrites for ci-cd-packaging-and-release-plan.md (484L → live-state pointer + Phase 2b/3 backlog index) + ci-troubleshooting.md (177L → release.yml + build-linux-container.yml topology + carry-forward unique tips); originals archived to `docs/_archive/` per the existing convention. **WSL Linux-only PATH gotcha** caught during P7 walkthrough (WSL `appendWindowsPath=true` puts `/mnt/c/Program Files/dotnet` ahead of `~/.dotnet` → child `dotnet pack` picks Windows binary, MSBuild fails MSB1001 on Linux paths) documented in cross-platform-smoke-validation.md + unix-smoke-runbook.md. Phase 2b tail unlocked: PA-2 behavioral validation, real release.yml hardening (first prerelease publication), `RemoteArtifactSourceResolver`, family/train orchestration. | Deniz İrgin + session collaboration |
| 2026-04-22 | v2.13 — Slice E follow-up pass kick-off (master-direct, post-`bfc6713`). Pass-1 merged, deferred Slice E scope (E1c/E1d/E3/E4/E6-tail/E7) restarts as master-direct commits per Deniz direction — branch overhead not worth it for CI-polish + witness work. Opened §2.2 with an 8-phase tracking table (P1 scaffolding → P8 retirement). **Design directions locked at P1 kick-off:** (1) Cake `EnsureVcpkgDependencies` target is local / witness-only; CI uses the existing `vcpkg-setup` composite action which already runs `vcpkg install` and populates `vcpkg_installed/<triplet>` (no doubled work), which explains why the first post-merge `workflow_dispatch` hit `Harvest precondition failed: vcpkg triplet directory ... is missing` — release.yml harvest body never invoked vcpkg-setup. (2) NuGet cache is cross-platform: `.nupkg` restores are RID-agnostic; single key family keyed on `hashFiles('**/packages.lock.json', '**/Directory.Packages.props', '**/*.csproj')` serves Windows / Linux / macOS (OS-aware path separators for operator-debuggability). (3) Cake host tests run in CI as part of `build-and-test-cake-host` (P2) with XPlat coverage collection + test-results artifact + `Coverage-Check` Cake-target gate — 426/426 local-host coverage becomes CI-backed. (4) prepare-native-assets-*.yml retirement (E3 / P8) is last — every other phase must witness green across all 3 platforms first. **P1 landed:** `.github/actions/nuget-cache/action.yml` + `.github/actions/platform-build-prereqs/action.yml` (macOS brew-only; Linux + Windows no-ops because custom GHCR image + `IMsvcDevEnvironment` cover their respective surfaces) + `Build.Tests.csproj` added to `build/build.sln` under `_build` solution folder (was missing; `dotnet test build/build.sln -c Release` now 426/426 green solution-wide, 593ms on Windows host). Node.js 20 deprecation pass through all workflows: `actions/checkout@v6`, `actions/setup-dotnet@v5`, `actions/cache@v5`, `actions/upload-artifact@v7`, `actions/download-artifact@v8`, `docker/build-push-action@v6` (with `provenance: false + sbom: false` to stop attestation manifests from tripping GHCR retention, which had pruned the amd64 runtime manifest on the first `build-linux-container` run and 404'd `docker pull`). `build-linux-container.yml` retention reworked to `delete-only-untagged-versions: true + min-versions-to-keep: 5`. `manifest.runtimes[linux-x64 + linux-arm64].container_image` pointed at `ghcr.io/janset2d/sdl2-bindings-linux-builder:focal-latest` (multi-arch OCI image index, successfully pushed after docker/build-push-action@v6 + provenance disable fixes). Dockerfile `FROM ubuntu:20.04` (debian:buster retired after first build hit EoL archive-move 404s; focal matches `prepare-native-assets-linux.yml`'s base exactly, glibc 2.31, active LTS). | Deniz İrgin + session collaboration |
