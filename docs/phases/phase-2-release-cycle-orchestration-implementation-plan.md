...
| **Slice D** — NativeSmoke extraction + Cake-native infrastructure + diagnostics | **LANDED (WIP)** | `1ef3a82` | Slice-D code commit on `feat/adr003-impl` pre-witness checkpoint. 390/390 tests green on Windows. Witness (D.15) + closure (D.16) deferred to follow-up sessions per Deniz 2026-04-21 direction. |
| **Slice DA** — Smoke playbook Cake-first alignment | **IN PROGRESS** | *WIP on `feat/adr003-impl`* | Doc-only slice ahead of witness. Three files in scope: `docs/playbook/cross-platform-smoke-validation.md` (A–K + PA-2 swept to Cake-first; bootstrap exception annotated; new checkpoints N/O/P added; stage-ownership mapping updated with NativeSmoke + diagnostics row), new `docs/playbook/unix-smoke-runbook.md` (unified Linux + macOS operational witness runbook, platform-param), retire `docs/playbook/TEMP-wsl-smoke-commands.md` + `docs/playbook/TEMP-macos-smoke-commands.md`. Broader doc sweep (AGENTS.md, onboarding.md, cake-build-architecture.md, release-guardrails.md, release-lifecycle-direction.md, phase-2-adaptation-plan.md, plan.md) deferred to Slice E per §6.6. |
| **Slice B2**, **Slice C**, **Slice E** | not started | — | — |
...
- [ ] D.15 — **Linux witness (WSL).** Deferred to a separate session per Deniz 2026-04-21 direction: commit + push WIP → `git pull` on the WSL-native clone (which carries a populated `vcpkg_installed/x64-linux-hybrid`) → `--target CleanArtifacts` → `--target SetupLocalDev --source=local --rid linux-x64` → standalone `--target NativeSmoke --rid linux-x64` → `--target Inspect-HarvestedDependencies --rid linux-x64`. Witness outcome amends §2.1 + §14 before slice close.
- [ ] D.16 — Slice D worktree health check green + committed as single atomic slice commit.
