...
| **P8** — Retirement + doc sweep tail (E3 + E6) | not started | — | Delete `.github/workflows/prepare-native-assets-{linux,macos,windows,main}.yml` + `.github/workflows/release-candidate-pipeline.yml` (placeholder stub). Canonical-doc broader sweep tail: `AGENTS.md`, `onboarding.md`, `knowledge-base/cake-build-architecture.md` surface drift, §11 Q17 ADR-002 §2.3 delegate-hook stance resolution. **Slice E follow-up pass closure commit.** |
