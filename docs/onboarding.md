...
PackageConsumerSmoke (matrix re-entry, per-TFM TUnit) → validates the produced nupkgs end-to-end
    ↓
PublishStaging → internal GitHub Packages feed (dispatch checkbox-gated; release tags auto-publish after trigger-aware version routing)
    ↓
...
- **Lock-file discipline** (P5, revised 2026-05-03): `<RestorePackagesWithLockFile>true</...>` + strict mode (`<RestoreLockedMode Condition="'$(GITHUB_ACTIONS)'='true'"/>`) apply only to `build/_build` + `build/_build.Tests`. `src/**` projects intentionally do not generate lock files because SDK-implicit packages drift with runtime/host patches (ILLink.Tasks per runtime patch, NETFramework.ReferenceAssemblies per host OS).
...
- **NuGet publishing**: `PublishStaging` is live for the internal GitHub Packages feed; dispatch stays gated behind `workflow_dispatch.inputs.publish-staging=true`, while release tags auto-publish to staging after trigger-aware `ResolveVersions` routing and smoke. `PublishPublic` is still stubbed pending PD-7 (nuget.org promotion via Trusted Publishing OIDC).
