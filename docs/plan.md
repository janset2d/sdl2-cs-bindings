...
**Last updated**: 2026-04-21 (ADR-003 direction pass + canonical doc sweep)
...
- **Phase 2b — Full Hybrid Pipeline + Orchestration Rewrite** (ACTIVE): Canonical doc sweep (current) → Cake refactor per ADR-003 (IPackageVersionProvider + per-stage request records + NativeSmoke extract + G58 + --explicit-version) → CI/CD workflow rewrite (`release.yml` with dynamic matrix + consumer smoke re-entry) → PA-2 behavioral validation on remaining 4 RIDs → Remote artifact source profile → public prerelease.
...
| **Artifact Source Profile abstraction (ADR-001 2026-04-18)** | `dotnet cake --target=SetupLocalDev --source=local\|remote`. `--source=local` runs repo pack → populates `artifacts/packages` → writes `build/msbuild/Janset.Smoke.local.props` (gitignored, consumer override). `--source=remote` (Phase 2b) fetches prebuilt nupkgs from internal feed → populates local cache → writes same override file. Consumer-side contract is identical either way (PackageReference from local folder feed). Cake abstraction: `IArtifactSourceResolver` + `ArtifactProfile { Local, RemoteInternal, ReleasePublic }`. Local resolver lands Phase 2a; RemoteInternal/ReleasePublic stubbed contract-only (implementation Phase 2b Stream D-ci). | Single orchestration surface principle (Cake-owned) with abstraction point at feed-prep, not consumer behaviour. CI and local dev converge on one mental model. Stream D-ci implementation plugs into existing resolver contract. See [ADR-001 §2.7–§2.8](decisions/2026-04-18-versioning-d3seg.md). |
...
| 2b | Full Hybrid Pipeline + Orchestration Rewrite | **ACTIVE** | Complete ADR-003 implementation: all 7 RIDs, all 6 satellites, Cake refactor, CI guardrails, consumer-smoke matrix re-entry, release pipeline |
...
- [x] **PA-2: Hybrid overlay triplet expansion** — landed 2026-04-18: added overlay triplets for the remaining 4 RIDs, moved all 7 `manifest.json` runtime rows to `hybrid-static`, and wired CI's shared vcpkg setup plus the manual orchestrator workflow to use hybrid triplets. End-to-end pack/consumer validation for the four newly-covered rows is still pending 2b.
...
- [ ] Stream F: Local-dev feed preparation + artifact source profiles — Cake two-source framework (`--source=local|remote`). `SetupLocalDev --source=local` is landed for the host RID end to end (packages local feed + writes `build/msbuild/Janset.Smoke.local.props` for IDE-ready smoke restore/build). `--source=remote` interface is wired, producer/URL/auth/cache mechanism open, full impl 2b. Historical Source Mode mechanism is retired by ADR-001; PD-5 is reframed as RemoteInternal profile implementation.

**Active — ADR-003 Orchestration Rewrite (2026-04-20 draft):**

ADR-003 locked the post-sweep direction (three `IPackageVersionProvider` impls, stage-owned validation, consumer smoke matrix re-entry, Option A resolver-centric `SetupLocalDev`, PD-7/PD-8/PD-13 direction-selected, G58 new). The implementation sequence is 4 separately-revertable commit groups; canonical doc sweep is step 1.

- [~] **Canonical documentation sweep** (ACTIVE 2026-04-21) — align all canonical docs with ADR-003 baseline, rewrite `phase-2-adaptation-plan.md`, retire-to-stub `phase-2-cicd-packaging.md` + `phase-3-sdl2-complete.md`, narrow `release-lifecycle-direction.md` to policy-only, update `cake-build-architecture.md` + `release-guardrails.md` + `ci-cd-packaging-and-release-plan.md` + `cross-platform-smoke-validation.md`, refresh `plan.md` + `docs/README.md` + `phases/README.md`. Historical pre-sweep body of `phase-2-adaptation-plan.md` preserved in [_archive/phase-2-adaptation-plan-2026-04-15.md](_archive/phase-2-adaptation-plan-2026-04-15.md).
- [ ] **Cake refactor** (2-3 sessions) — `IPackageVersionProvider` + 3 impls; per-stage request records; extract `NativeSmokeTask` from Harvest; `PackageTask` input refactored to per-family version mapping; `G58` validator in Pack stage; `--family-version` retire + `--explicit-version` introduce (PD-13 closure); `SetupLocalDev` retained as Option A thin task; test suite adjusted + new provider / G58 tests.
- [ ] **CI/CD workflow rewrite** (1-2 sessions) — new `release.yml` with tag + `workflow_dispatch` triggers, dynamic matrix from `manifest.runtimes[]` via `GenerateMatrixTask`, consumer smoke matrix re-entry; deprecation or harvest-only reuse of existing `prepare-native-assets-*.yml`.
- [ ] **PA-2 behavioral validation via new pipeline** — `workflow_dispatch` on `win-arm64` / `win-x86` / `linux-arm64` / `osx-arm64`; `mode=manifest-derived`, `suffix=pa2.<run-id>`; failure triage per playbook PA-2 section.
...
7. **Hybrid triplet expansion mechanism landed, behavioral coverage still pending**: all 7 runtime rows now map to hybrid overlay triplets in `build/manifest.json`, and CI's shared vcpkg setup/orchestrator workflow are aligned to that allocation. The remaining gap is behavioral validation on the four newly-covered rows (`win-x86`, `win-arm64`, `linux-arm64`, `osx-arm64`) on their native runners. See PA-2 status in [phases/phase-2-adaptation-plan.md](phases/phase-2-adaptation-plan.md#alignment-items-pre-stream-c).
...
11. **Release lifecycle direction locked, implementation partially designed**: Package family model, D-3seg tag-derived versioning, hybrid release governance, dynamic CI matrix, three-stage promotion, and the package-first consumer contract are all locked in `docs/knowledge-base/release-lifecycle-direction.md`. Implementation plan split into Streams A-safe / A-risky, A0, B, C, D-local / D-ci, E (scope-reduced), F — see `docs/phases/phase-2-adaptation-plan.md`. ADR-001 (2026-04-18) retired the old Source Mode mechanism: Stream F now covers feed preparation only. `SetupLocalDev --source=local` is landed and validated on `win-x64`, writing `build/msbuild/Janset.Smoke.local.props` so smoke csprojs restore/build directly in IDE or CLI; `--source=remote` remains open as PD-5's `RemoteInternal` artifact-source implementation. PD-2 is withdrawn under S1, PD-1 is resolved, PD-6 is closed as not applicable under the package-first contract, and PD-3 remains open — see the adaptation plan's Pending Decisions table.
...
19. **ADR-003 orchestration direction draft-locked (2026-04-20 v1.4)**: Three `IPackageVersionProvider` impls (Manifest / GitTag / Explicit), stage-owned validation (PreFlight / Harvest / NativeSmoke / Pack / ConsumerSmoke / Publish), consumer smoke matrix re-entry, Option A resolver-centric `SetupLocalDev`, `--family-version` retirement under PD-13, new guardrail G58 (cross-family dep resolvability, Pack stage). PD-7 / PD-8 / PD-13 direction-selected; formal closure lands with implementation. Canonical ADR: [`docs/decisions/2026-04-20-release-lifecycle-orchestration.md`](decisions/2026-04-20-release-lifecycle-orchestration.md). **Status: pseudocode / mental model — implementation pending.** 4-step impl sequence: (1) canonical doc sweep (active 2026-04-21), (2) Cake refactor, (3) CI/CD workflow rewrite, (4) PA-2 behavioral validation via new pipeline.
20. **Canonical doc sweep in progress (2026-04-21)**: Doc-level alignment pass for ADR-003 baseline. `phase-2-adaptation-plan.md` rewritten with historical body preserved at [`docs/_archive/phase-2-adaptation-plan-2026-04-15.md`](_archive/phase-2-adaptation-plan-2026-04-15.md). `phase-2-cicd-packaging.md` + `phase-3-sdl2-complete.md` retired to redirect stubs (roadmap-copy drift; unique content absorbed into `plan.md` roadmap + issue #60 samples spec). `phases/README.md` gains **Planned Phase Doc Retention Test** invariant to prevent future recurrence of the phase-3 drift pattern. `release-lifecycle-direction.md` narrowed to policy-only (orchestration devolved to ADR-003). `cake-build-architecture.md` + `release-guardrails.md` + `ci-cd-packaging-and-release-plan.md` + `cross-platform-smoke-validation.md` updated with ADR-003 stage-owned framing. New `docs/_archive/` convention documented in `docs/_archive/README.md`. PD-16 (shared native dep duplicate policy) opened in the rewritten adaptation plan, absorbed from retired `phase-2-cicd-packaging.md` §2.8.
21. **Build-host test suite: 340 tests green (measured 2026-04-20)**. Up from the earlier 247 / 256 / 273 milestones referenced in older handoff notes. Coverage ratchet policy active via `Coverage-Check` Cake task against `build/coverage-baseline.json`; CI gate wiring still deferred to Stream C PreflightGate.
