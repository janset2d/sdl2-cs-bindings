...
**Status (2026-05-20):** Generated SDL2.Core internal ABI surface is stabilized enough to advance to public typed wrappers and friendly overloads. The generator is Cake-hosted under `build/_build/Targets/GenerateBindings/`, Linux-canonical through `tools.cs generate-bindings`, and manifest-driven through `build/manifest.json` schema `2.2`. Source-first macro constants, semantic type classification, handles, enums, structs, callbacks, fixed-array policy, C `long` / `wchar_t` / SDL2 `SDL_bool` / `SDL_RWops` handling, and known bitmask enum fixes have fixture-backed coverage. Stage 1 remaining work is public typed low-level functions, friendly overloads, production output flip to `src/SDL2.Core/Generated/`, generated-source reproducibility/stamp gates, and package-consumer smoke. `SDL_syswm.h` typed-union layout remains Stage 2. SDL3 binding generation is gated on PD-7.
