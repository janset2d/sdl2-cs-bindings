...
**Active plan:** [`refactoring/target-centric-build-host-refactor-plan.md`](refactoring/target-centric-build-host-refactor-plan.md). ADR-002 P0-P4 closed. `Info`, `ResolveVersionsFromManifest`, `ResolveVersionsFromExplicit`, and the four diagnostic targets migrated; `versions.json` path contract resolved; `CleanArtifacts` and `CompileSolution` retired from Cake; `testing-guidelines.md` extracted; `docs/refactoring` lifecycle codified. P5 (strategy retirement) or P6 (PreFlight migration) is next.
