...
**Phase X / build-host refactor (ADR-002 target-centric architecture) — ACTIVE.** P0 (docs/guardrails), P1 (baseline), P2a (V2 test infra), P2b (foundation primitives), P3 (repositories + BuildContext + InfoTask migration), and the ResolveVersions part of P4 are complete. `versions.json` path contract resolved: `--versions-file` is universal for both ResolveVersions writers and stage-task readers, `PathService` no longer hardcodes the output directory. `CleanArtifacts`, `CompileSolution`, and `Coverage-Check` retired from Cake (CleanArtifacts: local hygiene owned by `tools.cs`; CompileSolution: zero callers, bare `dotnet build` covers the use case; Coverage-Check: ADR-002 §14 retirement in S10 2026-05-08, coverage instrumentation also removed from CI). `testing-guidelines.md` extracted as canonical test reference. ~529 tests, 0 failures. **P4 + P5 closed** (S10 + S11, both 2026-05-08) — all four diagnostic targets migrated; coverage retired entirely (gate, instrumentation, CI artifact); strategy abstraction retired (manifest field, Shared/Strategy/ folder, factories, single-impl interfaces, OneOf-shaped ValidationResult); G16 replaced by `HybridStaticOverlayValidator` validating triplet-overlay-file existence; G19 leak validator survives as `HybridStaticLeakValidator` in `Shared/Harvesting/` with canonical `ValidationReport` return shape. Next: P6 (PreFlight migration). Canonical docs: [`decisions/2026-05-05-target-centric-build-host.md`](decisions/2026-05-05-target-centric-build-host.md), [`refactoring/target-centric-build-host-refactor-plan.md`](refactoring/target-centric-build-host-refactor-plan.md), [`refactoring/target-centric-build-host-review-checklist.md`](refactoring/target-centric-build-host-review-checklist.md), [`refactoring/testing-guidelines.md`](refactoring/testing-guidelines.md).
...
- [ ] **GitHub Actions `windows-2025` runner deprecation** — `manifest.json runtimes[].runner` references `windows-2025` for `win-x64` and `win-x86` (`win-arm64` already uses `windows-11-arm`). GitHub Actions is sunsetting `windows-2025` by **2026-05-12**, redirecting to `windows-2025-vs2026`. Surfaced as a NOTICE annotation in the S11 release pipeline run (`25550425138`). Update the manifest entries before the deprecation date and re-run the full release pipeline to verify Harvest + ConsumerSmoke still pass on the new image.
