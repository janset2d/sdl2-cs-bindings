...
Strategy brief: [binding-autogen/binding-autogen-strategy-brief.md](binding-autogen/binding-autogen-strategy-brief.md) (revised through 2026-05-18).
...
Translation contract: [binding-autogen/binding-translation-contract.md](binding-autogen/binding-translation-contract.md) (canonical C-to-C# mapping rules, evidence gates, and current SDL2.Core P0 blockers).
...
**Status (2026-05-18):** Strategy + unified spec + unified plan accepted. Generator folded into the Cake build host under `build/_build/Targets/GenerateBindings/` (not a standalone `src/`-tree project). Linux-canonical determinism contract — Docker hard prereq for local invocation via `tools.cs generate-bindings`. Stage 1 must make SDL2.Core production-shaped before it stops compiling `external/sdl2-cs/src/SDL2.cs`: manifest-driven `SDL2.SDL` identity, critical function coverage, constants/enums, structs/unions, fixed arrays, callbacks, and compile-check trust gates. `SDL_syswm.h` typed-union layout remains Stage 2. API surface locked as internal raw ABI externs + public typed low-level API + friendly overloads; public raw `IntPtr` externs are not in v1 preview; SDL2-CS compatibility is best-effort. String-like SDL macro constants use canonical `ReadOnlySpan<byte>` UTF-8 literal properties, while `string` ergonomics belongs to method overloads. Fixed arrays now prefer package-TFM-safe output: primitive buffers emit C# `fixed` buffers, non-fixed-buffer arrays emit generated wrapper structs instead of `[InlineArray]`, and the compile-check project builds the generated preview across `LibraryTargetFrameworks` (`net10.0;net9.0;net8.0;netstandard2.0;net462`). SDL3 binding generation gated on PD-7.
...
- SDL2.Core P0 translation blockers now have fixture-backed fixes for opaque handle aliases, platform-sensitive C integers, `wchar_t*` plus erased HID wide-string pointers, `SDL_RWops` quarantine, platform-control macros, SDL2 `SDL_bool`, and known bitmask enums. `SDL_RWops` intentionally remains Stage 1 opaque pending full platform layout proof, and C `long` / `unsigned long` raw signatures are guarded to .NET 6+ via `CLong` / `CULong`.
