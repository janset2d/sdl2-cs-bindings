...
**Last updated**: 2026-05-04 (Phase Y Wave A+B CLOSED; Phase X build-host modernization wave: P0 + P1 + P2 + Adım 13 + P3 + P4-A CLOSED on `master` at `d1127e4` — ADR-002 layered shape retired, ADR-004 5-folder shape live; P4-C large Pipeline decomposition + P5 not started)
...
| --- | --- | --- | --- |
| P0 Safety Baseline | ✅ CLOSED | `e602b6c` | smoke-witness `--emit-baseline` flag + verify-baselines.cs file-based-app helper + 4 baselines (Win local fast-loop, Linux + ci-sim Win + macOS Intel milestone-loop) + 6 doc updates + ADR-004 §2.14 rename criterion. Behaviour signal locked at this commit. **Note:** The smoke-witness and verify-baselines.cs baseline scaffolding was retired in Phase Y (2026-05-03); verification logic moved to `tools.cs`. |
...
| Phase Y Wave B | ✅ CLOSED | — | Dev orchestration extraction: move setup/test/package workflows from Cake host into `tools.cs` repo-root file-based app; Cake host narrows to CI-only mission. `SetupLocalDev` (including `SetupLocalDevFlow`, `IArtifactSourceResolver`, and artifact source resolvers) retired from the Cake host. |

Phase X and Phase Y waves are closed — Phase X P5 (target-rename atomic) remains as the sole open item.

See [phases/README.md](phases/README.md) for the full phase breakdown and [phases/phase-2-adaptation-plan.md](phases/phase-2-adaptation-plan.md) for the Phase 2b active execution ledger.
...
| **Artifact Source Profile abstraction (ADR-001 2026-04-18; operational surface updated Phase Y 2026-05-03)** | `tools setup --source=local` (repo pack → `artifacts/packages` → `Janset.Local.props`) and `tools setup --source=remote-github` (GH Packages download → same override file). Consumer-side contract unchanged (PackageReference from local folder feed). The original Cake abstraction (`IArtifactSourceResolver` + `ArtifactProfile`) was retired in Phase Y; the feed-prep seam now lives in repo-root `tools.cs`. `ReleasePublic` stays stubbed pending Phase 2b PD-7 (nuget.org promotion). | Single orchestration surface with abstraction at feed-prep, not consumer behaviour. See [ADR-001 §2.7–§2.8](decisions/2026-04-18-versioning-d3seg.md). |
| **Cake-native feature-oriented build-host architecture (ADR-004 2026-05-02; supersedes ADR-002 DDD layering of 2026-04-19)** | Five top-level folders: `Host/` (Cake/Frosting runtime, CLI, BuildContext, composition root, paths, Cake extensions) + `Features/<X>/` (operational vertical slices — Cake Task + size-triggered Pipeline + validators + generators + Request DTOs + per-feature `ServiceCollectionExtensions`) + `Shared/` (build-domain vocabulary — no Cake deps, no I/O) + `Tools/` (Cake `Tool<TSettings>` wrappers ONLY) + `Integrations/` (non-Cake-Tool external adapters). Direction-of-dependency invariants enforced by `ArchitectureTests` (renamed from `LayerDependencyTests` at the P2 wave). Pipeline extraction is **size-triggered** (~200 LOC smell threshold). `BuildContext` is invocation state, not service locator. Interface discipline (3 criteria): multiple impls, independent axis of change, or transitional high-cost test seam. Cake host is CI-only; dev orchestration lives in repo-root `tools.cs`. | Cognitive routing matches operational journey. Cross-feature data flows through `Shared/` (strict, no exceptions — the LocalDev orchestration carve-out was retired in Phase Y). All migration waves (P0–P4-A) closed. [ADR-004](decisions/2026-05-02-cake-native-feature-architecture.md). |
...
- [x] Stream F: Local-dev feed preparation + artifact source profiles — Cake two-source framework (`--source=local|remote`, landed for all 3 maintainer hosts 2026-04-29). **Phase Y (2026-05-03) retired the Cake-host implementation** (`SetupLocalDev` task, `SetupLocalDevFlow`, `IArtifactSourceResolver`, and artifact source resolvers). Feed-prep orchestration moved to `tools.cs` as `tools setup --source=local|remote-github|remote-nuget`. The `tools setup --source=remote-github` path (against GitHub Packages) is operational and witnessed; `--source=remote-nuget` (public NuGet.org) stays stubbed pending Phase 2b PD-7. Historical Source Mode mechanism is retired by ADR-001.
...
- [x] Implement `RemoteArtifactSourceResolver` concrete (PD-5): internal feed URL convention (`https://nuget.pkg.github.com/janset2d/index.json`), auth pattern (`GH_TOKEN → GITHUB_TOKEN` env chain, Classic PAT with `read:packages`), cache strategy (`artifacts/packages/` with wipe-on-prepare). `SetupLocalDev --source=remote` operational and witnessed on Windows `win-x64`, WSL Linux `linux-x64`, and macOS Intel `osx-x64` against CI run 24962876812. **Note:** The Cake-host resolver implementation was retired in Phase Y (2026-05-03); the remote feed-prep seam moved to `tools.cs` as `tools setup --source=remote-github`.
...
- **Architecture Decision Records**: [decisions/](decisions/) — [ADR-001 D-3seg + package-first + artifact source profile](decisions/2026-04-18-versioning-d3seg.md), [ADR-002 DDD layering for build host](decisions/2026-04-19-ddd-layering-build-host.md) (superseded by ADR-004), [ADR-003 release lifecycle orchestration + version source providers](decisions/2026-04-20-release-lifecycle-orchestration.md), [ADR-004 Cake-native feature-oriented build-host architecture](decisions/2026-05-02-cake-native-feature-architecture.md)
