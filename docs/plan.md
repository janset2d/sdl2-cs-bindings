...
- [ ] **Unify diagnostic target UX** — bring `Dumpbin-Dependents` and `Ldd-Dependents` up to `Otool-Analyze`'s elaboration (per-platform system-library classifier, dependency table, manifest.json `system_exclusions` suggestion section). Consume `Build.DependencyAnalysis.{WindowsDumpbinScanner, LinuxLddScanner, MacOtoolScanner}` (root concept post-S16) for DRY with Harvest.
