...
> **Next:** Stream A-risky (MinVer + exact-pin csproj rollout + structural lock — now unblocked by A0 mechanism proof). HarvestPipeline extraction (#87) is intentionally deferred and not the default next task. Stream C (PreFlightGate expansion + dynamic matrix + CI artifact-flow) is gated on PA-1 and PA-2.
...
- [x] Stream A0: Exact pin spike — mechanism proven, PD-2 resolved (`PrivateAssets="all"` + explicit `PackageReference` with bracket notation). First automated guard lands in A-risky (PreFlight csproj structural validator); second guard lands in D-local (post-pack nuspec assertion). See [research/exact-pin-spike-and-nugetizer-eval-2026-04-16.md](research/exact-pin-spike-and-nugetizer-eval-2026-04-16.md)
- [x] Stream A-risky: MinVer 7.0.0 + exact-pin csproj rollout (Mechanism 3) + family identifier rename to canonical `sdl<major>-<role>` + MSBuild guard target + PreFlight `CsprojPackContractValidator` (G1-G8 + G17-G18) — landed 2026-04-16. PD-1 resolved, PD-9 opened. 256/256 build-host tests green. See [phases/phase-2-adaptation-plan.md Stream A-risky](phases/phase-2-adaptation-plan.md)
...
11. **Release lifecycle direction locked, implementation partially designed**: Package family model, tag-derived versioning, hybrid release governance, dynamic CI matrix, three-stage promotion — all locked in `docs/knowledge-base/release-lifecycle-direction.md` (including the "Two Version Planes" section clarifying upstream library version vs family version, and the canonical `sdl<major>-<role>` family identifier convention adopted 2026-04-16). Implementation plan split into Streams A-safe / A-risky, A0, B, C, D-local / D-ci, E (scope-reduced), F — see `docs/phases/phase-2-adaptation-plan.md`. **PD-4 mechanism locked 2026-04-15** — verified on Windows (worktree), Linux (WSL Ubuntu, 2-level chain), and macOS (SSH Intel Mac, Darwin 24.6, 3-level dylib chain); platform-branched (`<Content>` on Windows, `<Exec cp -a>` on Unix, symlink chains preserved at 1× size); end-to-end validation with real SDL2 natives pending Stream F execution — see [`docs/research/source-mode-native-visibility-2026-04-15.md`](research/source-mode-native-visibility-2026-04-15.md). **PD-5 direction locked 2026-04-15** via the same doc's two-source framework (`--source=remote --url=<url>`); concrete mechanism (URL convention, producer workflow, auth, caching) unresolved — 2b scope. **PD-2 resolved 2026-04-16** — `PrivateAssets="all"` on Native ProjectReference + explicit `PackageReference` with bracket notation; empirically verified on .NET SDK 9.0.309, 4 TFMs, parameterized versions; LibGit2Sharp production precedent; A0 spike closed, first automated guard lands in A-risky (PreFlight csproj structural validator), second in D-local (post-pack nuspec assertion) — see [`docs/research/exact-pin-spike-and-nugetizer-eval-2026-04-16.md`](research/exact-pin-spike-and-nugetizer-eval-2026-04-16.md). **PD-1 resolved 2026-04-16** — MinVer 7.0.0 sets `$(Version)` correctly on native csprojs even with `<IncludeBuildOutput>false</IncludeBuildOutput>`; empirically verified during A-risky implementation; pack of `Janset.SDL2.Image.Native` produces correctly-versioned nuspec from MinVer-derived version. **PD-3, PD-6 still open; PD-5 direction locked with mechanism open** — see adaptation plan's Pending Decisions table.
...
14. **Within-family exact-pin auto-derivation from MinVer is at the frontier (PD-9, opened 2026-04-16)**: MSBuild item static-evaluation timing locks `PackageVersion.Version` metadata before MinVer's targets fire; restore-time hook chicken-and-egg with MinVer package targets loading. Standalone `dotnet pack` produces sentinel `[0.0.0-restore]`, blocked from shipping by `_GuardAgainstShippingRestoreSentinel` MSBuild guard in [`src/Directory.Build.targets`](../src/Directory.Build.targets). Industry survey (LibGit2Sharp hardcodes literal, SkiaSharp uses minimum range, Avalonia minimum range, Magick.NET hardcodes, SDL3-CS bundles): no major .NET monorepo solves this. Production path = Cake-driven two-step orchestration (Stream D-local). PD-9 tracks ecosystem evolution as "watch for"; not blocking. See [`docs/research/exact-pin-spike-and-nugetizer-eval-2026-04-16.md` Part 3](research/exact-pin-spike-and-nugetizer-eval-2026-04-16.md#part-3-production-time-version-flow-constraint-empirical-finding-2026-04-16) and PD-9 in adaptation plan.
15. **Comprehensive guardrail roadmap published (2026-04-16)**: 45 guardrails enumerated across 8 categories (csproj structural, MSBuild build-time, NuGet built-in, manifest+vcpkg coherence, post-pack nuspec assertion, CI pipeline, full-train validation, manual escape hatch). Each mapped to its owning stream + current implementation status. Defense-in-depth principle: critical invariants are checked at multiple layers (e.g., within-family exact pin checked at G1+G2+G3 structural, G9 build-time, G20+G24 post-pack). Failure mode catalog cross-references guardrails to known failure modes — every cataloged failure has at least one covering guardrail. New canonical doc: [`docs/knowledge-base/release-guardrails.md`](knowledge-base/release-guardrails.md). Used as the single map for "which guardrail lives where + when does it land."
