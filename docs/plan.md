...
| **Cake-native feature-oriented build-host architecture (ADR-004 2026-05-02; supersedes ADR-002 DDD layering of 2026-04-19)** | Five top-level folders: `Host/` (Cake/Frosting runtime, CLI, BuildContext, composition root, paths, Cake extensions) + `Features/<X>/` (operational vertical slices — Cake Task + size-triggered Pipeline + validators + generators + Request DTOs + per-feature `ServiceCollectionExtensions`; `Features/LocalDev/` is the designated multi-feature orchestration slice) + `Shared/` (build-domain vocabulary — manifest models, runtime types, version mapping, package family conventions, cross-feature result primitives — no Cake deps, no I/O) + `Tools/` (Cake `Tool<TSettings>` wrappers ONLY) + `Integrations/` (non-Cake-Tool external adapters — NuGet protocol client, dotnet pack invoker, project metadata reader, coverage XML readers, vcpkg manifest reader, MSVC env resolver). Direction-of-dependency invariants enforced by `ArchitectureTests` (renamed from `LayerDependencyTests` at the P2 wave). Pipeline extraction is **size-triggered** (~200 LOC smell threshold, not hard rule). `BuildContext` is invocation state, not service locator: Pipelines target `RunAsync(TRequest)`, services take explicit inputs, Tools/Integrations take narrow Cake abstractions. Interface discipline (3 criteria): multiple impls, independent axis of change, or transitional high-cost test seam — test mocks alone insufficient. | Cognitive routing matches operational journey, not domain model. Adding a satellite touches one feature folder, not three layers. Cross-feature data flows through `Shared/` (architecture-test invariant #4 with single allowlist exception for `Features/LocalDev/`). Migration in progress (P0–P5 waves) — see [phase-x-build-host-modernization-2026-05-02.md](phases/phase-x-build-host-modernization-2026-05-02.md). [ADR-004](decisions/2026-05-02-cake-native-feature-architecture.md). |
