...
- [ ] Stream F: Local-dev feed preparation + artifact source profiles — Cake two-source framework (`--source=local|remote`). `SetupLocalDev --source=local` is landed for the host RID end to end (packages local feed + writes `build/msbuild/Janset.Local.props` for IDE-ready smoke restore/build). `--source=remote` interface is wired, producer/URL/auth/cache mechanism open, full impl 2b. Historical Source Mode mechanism is retired by ADR-001; PD-5 is reframed as RemoteInternal profile implementation.
