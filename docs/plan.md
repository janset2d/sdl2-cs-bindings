...
| 2b | Full Hybrid Pipeline + Orchestration Rewrite | **ACTIVE** | Slice A→C landed on master 2026-04-22 (`bfc6713`): Cake refactor complete, three `IPackageVersionProvider` impls, per-stage request records, stage-owned validation, flat task graph, G58 Pack-stage gate, D-3seg versioning, package-first consumer contract, three-platform smoke witness green, 426/426 tests. **Slice E follow-up pass open** (CI absorption: prepare-*.yml → release.yml composite actions + GHCR image push + lock-file + PublishTask stubs + three-platform CI witness). PA-2 behavioral validation on 4 newly-hybridized RIDs + NuGet publish remain Phase 2b tail. |
...
- [~] **CI/CD workflow rewrite** — partial. `release.yml` grew through every slice: stub `resolve-versions-stub` (A) → real `resolve-versions` + `preflight` (B1) → `generate-matrix` + first-draft `harvest` / `native-smoke` / `consolidate-harvest` (D) → real `pack` + `consumer-smoke` + stubbed `publish-staging` / `publish-public` (C) → `build-cake-host` artifact + all consumers consume `dotnet ./cake-host/Build.dll` instead of per-job restore+build (E1a-b). `docker/linux-builder.Dockerfile` + `.github/workflows/build-linux-container.yml` landed but **image not yet pushed** to GHCR (mid-slice manual gate). Current Slice E follow-up working tree also closes two post-P4 investigations: `PackageConsumerSmoke` owns `win-x86` runtime bootstrap via Cake (`IDotNetRuntimeEnvironment` / `DotNetRuntimeEnvironment` installs x86 runtimes per executable TFM and injects `DOTNET_ROOT_X86` + `DOTNET_ROOT(x86)` into child `dotnet test` only), and `vcpkg-setup` now resolves mutable `container_image` tags such as `focal-latest` to immutable manifest-list digests inline for the cache key while the actual job still executes on the convenience tag; the same action now branches on `runner.os` instead of the stale literal `windows-latest` check. **Follow-up pass scope:** E1c (absorb `prepare-native-assets-*.yml` body into `release.yml` matrix jobs via platform-prereq composite actions + reuse of `vcpkg-setup`), E1d (`packages.lock.json` + `--locked-mode` discipline), E3 (retire `prepare-native-assets-*.yml`), E4 (`PublishTask` stubs), E5 (final lint pass), E6 (broader doc sweep + §11 Q17 ADR-002 §2.3 delegate-hook stance), E7 (three-platform witness per `cross-platform-smoke-validation.md` A-K checkpoints).
- [ ] **PA-2 behavioral validation via new pipeline** — `workflow_dispatch` on `win-arm64` / `win-x86` / `linux-arm64` / `osx-arm64`; `mode=manifest-derived`, `suffix=pa2.<run-id>`; failure triage per playbook PA-2 section. Executable only after Slice E follow-up pass closes and GHCR image is operational.
