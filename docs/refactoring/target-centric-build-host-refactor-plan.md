...
| ~~`build/_build/Features/Packaging/PackagePipeline.cs`~~ | Retired in S13 (2026-05-09). Pack migrated to `Targets/Package/`; orchestration owned by `PackageTask` directly with per-family `PackageFamilyPacker`. |
...
| `Validation` | All cross-cutting validators (manifest invariants, version consistency, core identity, csproj pack contract, upstream alignment, cross-family resolvability, hybrid-static overlay, harvest readiness, package output, native package metadata, README mapping table) under domain alt-folders (`Manifest/`, `Versioning/`, `Packaging/`, `Models/`, `Conventions/`). Single `AddValidators()` registration point. Established by S12 with seven validators; expanded to eleven by S13 (`HarvestReadinessValidator`, `PackageOutputValidator`, `NativePackageMetadataValidator`, `ReadmeMappingTableValidator`). | Target-specific orchestration; that lives in the consuming target (e.g. `PreFlightCheckTask`, `PackageTask`). |
...
> **Status:** P7 closed in S13 (2026-05-09). Pack-stage orchestration migrated to `Targets/Package/`; `PackagePipeline` + `IPackagePipeline` deleted; per-family flow extracted into `PackageFamilyPacker` (3-phase: EnsureHarvestReady → PrepareMetadata → PackAndValidate). New collaborators: `DependencyRangeNormalizer` (G56 zip-rewrite), `PackageReporter` (logging consolidation), `PackageFamilyPacker`. `DotNetPackInvoker` relocated target-local from `Integrations/DotNet/` to `Targets/Package/Services/`. New cross-cutting validators added under root `Validation/Packaging/`: `HarvestReadinessValidator` (manifest + consolidation receipt + payload subtree gates), the relocated `PackageOutputValidator`, plus the relocated `NativePackageMetadataValidator` (G55) and `ReadmeMappingTableValidator` (G57) extracted from their parent files in `Features/Packaging/` (models + generators stayed). `IPackageOutputValidator.ValidateAsync` reshaped to return `ValidationReport` directly. ADR-002 §8 amendment broadened from "the seven S12 validators" to "all build-host validators under root `Validation/`" (now 11 validators). 3 OneOf result types retired (`DotNetPackResult`, `ProjectMetadataResult`, `PackageValidationResult`); `PackageValidation` / `PackageValidationCheck` / `GuardrailKind` enum collapsed to `ValidationReport` / `ValidationCheck` (Code carries guardrail ID strings). `PackRequest` renamed to `PackageRequest` (relocated to `Targets/Package/Requests/`). V2 scenario coverage at `Scenarios/Package/PackageTaskScenarioTests.cs` (9 scenarios — happy path multi-family + 8 failure paths covering versions-file missing, G58 cross-family unresolved, harvest manifest missing, harvest receipt missing/zero-rids/zero-licenses, payload subtree missing, multi-error post-pack aggregation). V1 `PackagePipelineTests` retired; `PackageOutputValidatorTests` V2-migrated + relocated to `Unit/Validation/Packaging/` (15 tests, ValidationReport assertion shape with guardrail-ID `Code` queries). `DotNetPackInvokerTests` V2-migrated + relocated to `Unit/Targets/Package/Services/`. Test count: 525 → 544. Slopwatch: 0 issues. Local ci-sim: 8/8 PASS in 177s on Windows. Phase X items captured in [parking-lot.md](../parking-lot.md): `tools.cs` build-vs-cake naming, vcpkg cache key audit, Windows runner deprecation (carryover from S12); HEAD SHA Func<string> consistency revisit, `PackageFamilyPacker` ctor style consistency, follow-up V2 migration of `CsprojPackContractValidatorTests`.
...
> **Status:** P8 closed in S14 (2026-05-09). Harvest stack migrated to `Targets/{Harvest,NativeSmoke,ConsolidateHarvest}/`; `HarvestPipeline` (629L) + `NativeSmokePipeline` (238L) + `ConsolidateHarvestPipeline` (485L) all retired. `Targets/Harvest/` decomposes into `HarvestTask` (orchestration, MA0051-suppressed `ProcessLibraryAsync` with linear fail-fast steps) + `Services/{IBinaryClosureWalker, IArtifactPlanner, IArtifactDeployer}` (3 algorithm-bearing collaborators with interfaces) + `Reporting/HarvestReporter` (IAnsiConsole + ICakeLog cohort). `Targets/NativeSmoke/` collapses post-self-review into `NativeSmokeTask` only — Configure/Build/Execute/ApplyMsvcEnvironment as private methods after extract→re-inline pass per ADR §5; `INativeSmokeRunner` + `NativeSmokePrerequisiteResolver` deemed ceremonial wrapper-of-wrappers. `IMsvcDevEnvironment` + `MsvcTargetArch` relocated from `Shared/Runtime/` + `Integrations/Msvc/` dissolved. `Targets/ConsolidateHarvest/` decomposes the staged-replace pattern into `Services/{HarvestArtifactMerger, LicenseUnionWriter, StagedArtifactSwapper}` (each carrying a real algorithm — RID-status JSON load + manifest build, SHA256 divergence detection + per-RID variants, atomic file/dir swap with 2 overloads) + `Reporting/ConsolidateHarvestReporter`. 3 OneOf result types retired (`ClosureResult`, `ArtifactPlannerResult`, `CopierResult` → `Result<T,TError>`); surviving OneOf = 1 (`PackageInfoResult`, P9-bound). Validator cohort grew 11 → 14 with `Validation/Harvesting/{IHarvestPreconditionsValidator, IHybridStaticLeakValidator}` + `Validation/NativeSmoke/INativeSmokePreconditionsValidator`. ADR-002 §8 S14 amendment: parallel `Repositories/` cohort exception alongside S12's `Validation/` — all 4 repositories (`IManifestRepository`, `IVcpkgManifestRepository`, `IVersionFileRepository`, `IHarvestStatusRepository`) live under root `Build.Repositories` with `IFoo` interfaces + `AddRepositories()` registration + paired test classes (`<X>RepositoryUnitTests` mock + `<X>RepositoryRoundTripTests` sociable). `HarvestStatusRepository` retroactively relocated from `Targets/Harvest/Services/` to root `Repositories/`. **Configuration cleanup**: `DumpbinConfiguration` (zero consumers) + `Configurations` aggregate + `BuildContext.Options` (never read) all retired; `PackageFamilyPacker` `DotNetBuildConfiguration` injection (P7 leak) replaced with `context.BuildConfiguration` named property passed through `PackAsync`. Empty marker request types retired (`HarvestRequest`, `ConsolidateHarvestRequest`); `NativeSmokeRequest` kept (collaborator-consumed). `HarvestJsonContract` promoted to `Build.Harvesting` named concept; `Features/Harvesting/` folder dissolved entirely (`AddHarvestingFeature()` deleted). New V2 scenario coverage at `Scenarios/{Harvest,NativeSmoke,ConsolidateHarvest}/` (8 + 7 + 7 scenarios). 3 new DI smoke tests (`AddHarvest`, `AddNativeSmoke`, `AddConsolidateHarvest`). Multi-agent review (5 reviewers — full-context / necessary-context / unbiased-code-quality / unbiased-SOLID / test-discipline) ran at Phase 11; convergent BLOCK + Tier-2 SUGGEST findings absorbed before commit. Test count: 525 → 582. Slopwatch: 0 issues (baseline rebuilt at Phase 10). Local ci-sim: 8/8 PASS in 179.8s on Windows (parity with S13's 177s — zero pipeline regression). Phase X items captured in [parking-lot.md](../parking-lot.md): `ArtifactPlanner.CreatePlanAsync` 175-line statistics-derivation extract, `BinaryClosureWalker.BuildClosureAsync` two-BFS extract, `IPathService` god-service `IHarvestPaths` segregation, `HarvestStatusRepository` naming/scope reconsideration, ConsolidateHarvest aggregate log-and-throw amplification, V1 fixture migration for smoke tests + `MsvcDevEnvironmentTests` (deferred from this slice).
...
8. **JSON serialization contract consolidation.** Two `JsonSerializerOptions` instances coexist today: `Build.Harvesting.HarvestJsonContract.Options` (harvest-specific, snake_case + indented) and `Build.Host.Cake.CakeJsonExtensions.DefaultJsonOptions` (generic default). Naming policy is encoded as a runtime `JsonNamingPolicy` on every options instance instead of declared on the model types. Consolidate to a single root `Build.Json/` (or equivalent) namespace exposing **one** `JsonSerializerOptions` instance per shape (e.g. `Json.Default`, `Json.SnakeCase`), and move per-property naming onto the model types via `[JsonPropertyName]` / `[JsonConverter]` attributes so the on-disk shape is encoded in the type itself rather than implied by the options the caller happens to pass. Touches:
   ...
   - All harvest model types (`RidHarvestStatus`, `HarvestManifest`, `HarvestSummary`, `BinaryClosure`, `BinaryNode`, `ConsolidationState`, `DivergentLicense`, `HarvestStatistics`, `DeploymentStatistics`, package-info CLI output DTOs, etc.) — annotate properties with `[JsonPropertyName]` matching the current snake-case shape (or migrate to a model-level naming attribute / `JsonSourceGenerationOptions`) so the contract travels with the type.
   - All consumer sites (`HarvestStatusRepository`, `HarvestArtifactMerger`, Vcpkg package-info parsing, etc.) — drop the explicit `HarvestJsonContract.Options` argument; the type's attributes carry the shape.
