...
> - **`Features/<X>/`** — operational vertical slices (Ci, Coverage, DependencyAnalysis, Diagnostics, Harvesting, Info, Maintenance, Packaging, Preflight, Publishing, Vcpkg, Versioning). Each feature owns its Cake `Task`, size-triggered `Pipeline`, validators, generators, `Request` DTOs, and a per-feature `ServiceCollectionExtensions.AddXFeature(). The LocalDev feature was retired in Phase Y (2026-05-03); its orchestration moved to `tools.cs`.
...
- **LocalDev was the designated multi-feature orchestration slice** under ADR-004 §2.5 + §2.13 invariant #4 allowlist. The feature (including `SetupLocalDevFlow` and its `IArtifactSourceResolver` composition) was **retired in Phase Y (2026-05-03)**; its orchestration responsibility moved to the repo-root `tools.cs` as `tools setup --source=...`. The `SetupLocalDevTask` / `SetupLocalDevFlow` files are removed from the production tree.
...
- **Packaging flow** spans `Features/Packaging/`, `Features/Publishing/`, `Shared/Packaging/`, and `Integrations/DotNet/`: thin tasks (`PackageTask`, `PackageConsumerSmokeTask`, `PublishStagingTask`, `PublishPublicTask`) + size-triggered pipelines (`PackagePipeline`, `PackageConsumerSmokePipeline`, `PublishPipeline`) + narrow services (`DotNetPackInvoker`, `ProjectMetadataReader`, `PackageOutputValidator`, `G58CrossFamilyDepResolvabilityValidator`, `FamilyTopologyHelpers`). `SetupLocalDevTask` and `SetupLocalDevFlow` were **retired in Phase Y (2026-05-03)**; orchestration moved to `tools.cs`. `PackageOutputValidator` accumulates all post-pack observations (G21–G23, G25–G27, G46–G48, G55–G58) into a single `PackageValidation` aggregate so operators see the complete failure set, not first-throw-wins. **All 7 manifest runtime rows** green end-to-end via `release.yml` — Pack ✓ + ConsumerSmoke ✓ on master `8ec85c5` (CI run 24938451364, 2026-04-26). `PostFlightTask` retired in Slice B2 (2026-04-21) — semantics absorbed into standalone `Package` + `PackageConsumerSmoke` (Flow 2) per ADR-003 §3.3 + cross-platform-smoke-validation.md §K. `PublishStagingTask` is live for the internal GitHub Packages staging feed; dispatch is checkbox-gated and release tags publish after trigger-aware `ResolveVersions` routing. `PublishPublicTask` remains a Phase 2b PD-7 stub pending nuget.org Trusted Publishing OIDC.
...
- `INativeAcquisitionStrategy` was designed in the brief but never implemented; ADR-001's Artifact Source Profile abstraction (`tools setup --source=local|remote-github`) now covers that problem space from the feed-preparation side and was implemented in `tools.cs` during Phase Y (2026-05-03), retiring the earlier Cake-host IArtifactSourceResolver seam.
...
- **`<X>Flow`** (retired in Phase Y, 2026-05-03) — was a multi-feature orchestration slice. The only instance was `SetupLocalDevFlow`; the orchestration moved to `tools.cs` as `tools setup --source=...`.

Pipelines target `RunAsync(TRequest, CancellationToken)` — the ADR-004 §2.11.1 migration exception is closed at P4-A. Pipelines receive only their request DTO; Cake abstractions (`ICakeContext`, `ICakeLog`, `IPathService`) are constructor-injected. One known exception remains documented inline: `EnsureVcpkgDependenciesPipeline.Run(BuildContext)` (P4-deferred). Cake `Tool<TSettings>` wrappers under `Tools/` retain the `<X>Runner` suffix native to Cake (`LddRunner`, `OtoolRunner`, etc.) — see ADR-004 §2.10.
...
### SetupLocalDev composition (retired in Phase Y, 2026-05-03)

`SetupLocalDevFlow` (`Features/LocalDev/`) was the build host's only multi-feature orchestrator, composing `ManifestVersionProvider` + `EnsureVcpkgDependenciesPipeline` + `PreflightPipeline` + `HarvestPipeline` + `ConsolidateHarvestPipeline` + `IPackagePipeline` + `IArtifactSourceResolver.PrepareFeedAsync` into a single `SetupLocalDevTask` seam. The `IArtifactSourceResolver` hierarchy (`LocalArtifactSourceResolver`, `RemoteArtifactSourceResolver`, `ArtifactSourceResolverFactory`) dispatched the `--source=local|remote` profile.

All of the above were **retired in Phase Y (2026-05-03)**. The feed-prep orchestration moved to the repo-root `tools.cs` as `tools setup --source=local|remote-github|remote-nuget`. See [ADR-001 §2.7–§2.8](../decisions/2026-04-18-versioning-d3seg.md) for the historical resolver contract, and [AGENTS.md CLI reference](../../AGENTS.md) for current `tools setup` usage.
...
**CI-only scope (Phase Y, 2026-05-03):** the Cake Frosting build host (`build/_build/`) is a CI-only production pipeline for native harvesting, packaging, and validation. Day-to-day dev orchestration (setup, local test, package rehearsal) lives in `tools.cs`, the repo-root file-based .NET 10 app. See [AGENTS.md CLI reference](../../AGENTS.md) for `tools.cs` usage patterns.
...
│   ├── Packaging/          ← PackageTask + PackagePipeline + PackageConsumerSmokeTask/Pipeline + PackageOutputValidator + NativePackageMetadataGenerator + ReadmeMappingTableGenerator + SatelliteUpperBoundValidator + PackagingStrategyFactory + DependencyPolicyValidatorFactory + FamilyTopologyHelpers + JansetLocalPropsWriter + VersionsFileWriter + SmokeScopeComparator. **Phase Y retirement:** `SetupLocalDevTask` (registered here), `LocalArtifactSourceResolver`, `RemoteArtifactSourceResolver`, and `ArtifactSourceResolverFactory` were removed in Phase Y (2026-05-03) — the resolver seam moved to `tools.cs`.
...
| ~~`IArtifactSourceResolver`~~ | ~~`LocalArtifactSourceResolver` / `RemoteArtifactSourceResolver` / `ArtifactSourceResolverFactory` (Features/Packaging/ArtifactSourceResolvers)~~ | ~~Feed-prep profile selection per ADR-001 §2.7. Subsumed the retired `INativeAcquisitionStrategy`.~~ **Retired in Phase Y (2026-05-03)** — the consumer-feed seam moved to repo-root `tools.cs` as `tools setup --source=local|remote-github|remote-nuget`. |
...
| `IPackagePipeline` | `PackagePipeline` (Features/Packaging) | Per-family pack orchestration (renamed from `PackageTaskRunner` at P2); consumed by `PackageTask` |
