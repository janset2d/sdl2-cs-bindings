...
>
> **Last updated:** 2026-04-18 (D-3seg versioning adoption via ADR-001; PA-1 closure + PA-2 mechanism landing; see §3 Versioning Model, §4 validation state, and §5 CI matrix model)
>
...
>
> **Research basis:** Four independent research efforts converged on the prior (pre-D-3seg) conclusions; ADR-001 refined the versioning and consumer-contract decisions on 2026-04-18:
>
...
- Cake `PackageVersionResolver` accepts an explicit `--family-version=<semver>` CLI override for PD-8 manual-escape-hatch paths. Tag remains the source of truth for stable releases. **Note:** `SetupLocalDev` does not use the flag — it auto-derives per-family versions from `manifest.json library_manifests[].vcpkg_version` + local timestamp, and `PackageConsumerSmoke` trusts `Janset.Smoke.local.props` for smoke-path versions. The remaining justification for the flag is PD-8-only; future retirement is tracked as [PD-13](../phases/phase-2-adaptation-plan.md#pending-decisions).
...
4. **Tag-derived versions + orchestrator bootstrap override.** Family tags are the primary version source (read by MinVer at pack time). Cake's `PackageVersionResolver` accepts `--family-version=<semver>` as a bootstrap override for PD-8 manual-escape-hatch flows. Version numbers are never manually edited in project files. The tradeoff: stable releases require git tag discipline, and the `--family-version` CLI escape hatch is non-default + operator-gated. (*Current tooling: MinVer with `<MinVerTagPrefix>sdl<N>-<role>-</MinVerTagPrefix>` per family.*)

    > **`--family-version` future review (PD-13, opened 2026-04-19).** Post-D-3seg, the flag's surviving legitimate surface has narrowed to PD-8 manual-escape-hatch for single-family packs. `SetupLocalDev` auto-generates per-family versions from manifest upstream + local timestamp (does not read the flag). `PackageConsumerSmoke` was reconciled to trust `Janset.Smoke.local.props` as the version source of truth for smoke testing, so it no longer depends on the flag either. Release-path version source is the git family tag → MinVer. Under G54 + D-3seg, a single `--family-version` value cannot span multiple families (Core `2.32.x`, Image `2.8.x`, Gfx `1.0.x` each anchor their own UpstreamMajor.UpstreamMinor). The open question in PD-13: does the remaining PD-8 escape justify keeping the flag in `PackageVersionResolver` + public CLI surface, or should it be retired in favour of tag-push-then-pack / `--version-suffix` against manifest-derived upstream anchors? See [phase-2-adaptation-plan.md PD-13](../phases/phase-2-adaptation-plan.md#pending-decisions) for resolution criteria.
