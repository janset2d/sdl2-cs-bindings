...
- Cake `PackageVersionResolver` accepts an explicit `--family-version=<semver>` CLI override for bootstrap flows (`SetupLocalDev`) and PD-8 manual-escape-hatch paths. Tag remains the source of truth for stable releases.
