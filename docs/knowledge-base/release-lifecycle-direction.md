...
1. **Cake `PackageTask` is atomic per family.** Both managed and native nupkg for a family are emitted from a single task invocation at the same family version. There is no code path where one package is emitted at version `X` and its sibling at version `Y`. (The CLI input surface migrated to `--explicit-version <family>=<semver>` (or the `--versions-file` JSON form for CI flows) under PD-13 closure 2026-04-22 / Slice B1 — the legacy `--family-version` flag retired then; atomicity invariant is unchanged across the migration.)
...
**Validation state as of 2026-04-26:** all 7 manifest runtime rows green end-to-end via `release.yml` Pack ✓ + ConsumerSmoke ✓ on master `8ec85c5` (CI run 24938451364). The latest local A-K witness remains green on Windows + WSL Linux per the 2026-04-25 P7 walkthrough; macOS stays CI-only per Deniz direction (no local Mac hardware). The full post-S1 guardrail set (G21–G27, G46–G48, G54–G58) runs on every Pack invocation against the five concrete families (`sdl2-core`, `sdl2-image`, `sdl2-mixer`, `sdl2-ttf`, `sdl2-gfx`); placeholder families with null `managed_project`/`native_project` are filtered out automatically by `HasConcreteProjects` (current placeholder set: empty post-2026-04-22 sdl2-net retirement). See [`playbook/cross-platform-smoke-validation.md`](../playbook/cross-platform-smoke-validation.md) for the A-K checkpoint command reference + Last-validated header, and [`phase-2-release-cycle-orchestration-implementation-plan.md`](../phases/phase-2-release-cycle-orchestration-implementation-plan.md) §2.2 for the Slice E follow-up pass closure trail.
