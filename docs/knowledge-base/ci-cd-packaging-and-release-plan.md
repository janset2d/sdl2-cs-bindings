...
| `resolve-versions` | ADR-003 §3.1 entrypoint | Trigger-aware route: `ManifestVersionProvider` (`workflow_dispatch` `manifest-derived` + CI suffix), `ExplicitVersionProvider` (`workflow_dispatch` `explicit`), `GitTagVersionProvider` (family tag pushes + train tags); emits `versions.json` artifact for every downstream stage. |
