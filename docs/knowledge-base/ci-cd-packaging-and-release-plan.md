...
| `resolve-versions` | ADR-003 §3.1 entrypoint | Intended route: `ManifestVersionProvider` (`workflow_dispatch` `manifest-derived`), `GitTagVersionProvider` (tag pushes), `ExplicitVersionProvider` (`workflow_dispatch` `explicit`); emits `versions.json` artifact. Current gap: live workflow still hard-codes manifest-derived CI suffixes and needs trigger-aware routing before tag-push staging. |
...
| `publish-staging` | Internal staging feed push | Real implementation landed 2026-04-26; pushes managed + native nupkg pairs to GitHub Packages via `PublishStagingTask`. Still gated to `workflow_dispatch.inputs.publish-staging=true`; do not open tag-push auto-publish until `resolve-versions` emits tag-derived or explicit mappings correctly. |
...
**Triggers**: tag push (`sdl2-*-*.*.*` targeted family release; `sdl3-*-*.*.*` future SDL3 line; `train-*` full-train) + `workflow_dispatch` (mode=`manifest-derived`/`explicit` + optional `explicit-versions` inline list). **Routing rule:** tags and explicit dispatches must be converted into `ResolveVersions` provider calls first, then published through `versions.json`; gate-lifting `publish-staging` comes after that routing is live.
