...
| G60 | **Overlay port version coherence.** Every `vcpkg-overlay-ports/<port>/vcpkg.json` `version` + `port-version` MUST match the upstream port's `vcpkg.json` at the current `external/vcpkg` submodule pin (`external/vcpkg/ports/<port>/vcpkg.json`). vcpkg uses overlay-port versions silently when an overlay is active, so an upstream bump in the submodule that doesn't propagate to the overlay's `vcpkg.json` produces silent drift — the operator ships the old port version believing they shipped the new one. Validator iterates every overlay directory, parses both manifests, fails-closed on version mismatch / port-version mismatch / missing overlay manifest / missing upstream counterpart / invalid JSON. | Active | `OverlayPortVersionCoherenceValidator` (PreFlight) |
