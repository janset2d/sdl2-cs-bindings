...
| **PD-13 (`--family-version` retirement)** | **Direction selected.** Flag direction is retirement, replaced by `--explicit-version family=version,...` (ExplicitVersionProvider input) — type-safe, multi-family safe, G54-validated per entry. **Formal retirement** lands when `--explicit-version` is wired into `ExplicitVersionProvider` and the legacy `--family-version` flag is removed across all call sites during the code refactor pass. |
