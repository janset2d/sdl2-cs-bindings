...
| L | `SetupLocalDev --source=remote` | F | Remote artifact-source feed prep populates the local cache + writes `Janset.Local.props` + `versions.json` correctly; consumer smoke green against pulled feed | **Promotion criteria met 2026-04-26** (PD-5 closure, commits `bc7c677` + `fdabcae` + `549ad2f`). `RemoteArtifactSourceResolver` implemented; authenticated feed pull validated end-to-end via `tests/scripts/smoke-witness.cs remote` on `win-x64` (3/3 PASS, ConsumerSmoke 35/35). Linux + macOS host witnesses pending; structural move to Active table is a follow-up doc-hygiene pass. |
