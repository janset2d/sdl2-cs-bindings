...
**Last validated:** 2026-04-26 CI confirmation at master `8ec85c5` via `release.yml` run 24938451364; latest local walkthrough remains the 2026-04-25 P7 witness at `d190b5b` (Windows `win-x64` + WSL Linux `linux-x64`); macOS remains CI-only per Deniz direction (no local Mac hardware).
**Result (2026-04-26):** CI remains green after the post-closure smoke-doc alignment commit, while the latest local walkthrough remains the 2026-04-25 P7 witness:
...
**Scope note:** all 7 manifest runtime rows (`win-{x64,x86,arm64}` / `linux-{x64,arm64}` / `osx-{x64,arm64}`) green via CI matrix re-entry as of master `d190b5b`. The per-RID matrix re-entry shape from ADR-003 §3.4 + §3.5 is live in `release.yml` (`generate-matrix` → `harvest` matrix → `pack` → `consumer-smoke` matrix re-entry consuming the same `manifest.runtimes[]` source). PA-2 behavioral validation (formal witness pass with full guardrail-trail recording per the playbook) on the four newly-hybridized rows remains Phase 2b work. The local walkthrough below exercises a **single host RID per platform** (Windows + WSL Linux + macOS); CI dispatch is the full-matrix surface. PD-10 (consumer smoke `-r <rid>` vs default resolver path) deferred to Phase 2b.
