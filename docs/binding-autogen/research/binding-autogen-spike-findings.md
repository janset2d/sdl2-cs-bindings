...
>
> **2026-05-16 — Stage 1 Task 3.5 production findings appended as §11.** The original CppAst spike ran on a Windows host with a synthetic stub directory carrying `stddef.h`/`stdint.h`/`stdio.h` shims. Stage 1 Task 3.5 moved the generator into the pinned Linux-builder container; the C-stdlib shim approach was retired (CPATH resolves real glibc + GCC headers), but a different subset of stubs — Windows / Apple / WinRT platform headers — turned out to be necessary. New frictions, a defense-in-depth dynapi cross-check direction, and corrections to earlier assumptions are documented in §11.
...
> **Context:** The Stage 1 Task 3.5 slice (binding-generator local-output-loop precursor; design absorbed into [`docs/superpowers/specs/2026-05-16-binding-generator-unified-design.md`](../../superpowers/specs/2026-05-16-binding-generator-unified-design.md) §§13–14; historical: [`docs/superpowers/specs/superseded/2026-05-15-binding-generator-local-output-loop-design.md`](../../superpowers/specs/superseded/2026-05-15-binding-generator-local-output-loop-design.md)) implemented per-header `CppAst.CppParser.ParseFile` inside the pinned `linux-builder` Docker container, eight parse views, full SDL2.Core header surface (~58 headers after exclusions). The end-to-end smoke surfaced new frictions specific to running production scope on Linux, plus corrections to earlier assumptions in §7. This section is additive to §7.2 — original CppAst spike frictions remain authoritative for the spike-host setup; §11 records what changed when the generator moved into production lifecycle.

### 11.1 Iteration Log — Stage 1 Production Frictions

| # | Friction | Symptom | Resolution |
|---|---|---|---|
| 7 | **`CppParser.ParseFiles(all_88_headers)` collapses to ONE TU; per-header isolation needs the loop** | Parse fails: GCC intrinsic chain from `<xmmintrin.h>` etc. pollutes libclang's builtin function table when 88 SDL2 headers expand transitively into one synthetic root TU | Loop `CppParser.ParseFile(h)` per header. CppAst's `ParseFile` internally delegates to `ParseFiles([h])` (one libclang TU per call) — isolation comes from the **loop**, not the API choice. Peer convergence: `amerkoleci/Alimer.Bindings.SDL` uses identical pattern; `ppy/SDL3-CS` invokes `ClangSharpPInvokeGenerator --file <single>` in a loop. §7.2 frictions did not surface this because the SDL2_gfx spike only parsed 4 headers in one TU, narrow enough to escape. |
| 8 | **SDL_cpuinfo.h → GCC intrinsic header chain → libclang builtin-function collision** | `error: definition of builtin function '_mm_pause' / '_mm_getcsr' / '__rdtsc' / '_mm_clflush' / '_mm_{l,m,s}fence'` blocks every parse view that reaches `SDL_cpuinfo.h` | SDL2 ships documented escape hatches at `SDL_cpuinfo.h:118-133`: `SDL_DISABLE_{IMMINTRIN,MMINTRIN,XMMINTRIN,EMMINTRIN,PMMINTRIN,MM3DNOW,LSX,LASX,ARM_NEON}_H`. Defining the full family as parser-level `Defines` short-circuits the entire `#include <*intrin.h>` block. This is the Linux-ecosystem analogue of §7.2 friction #2 (`__PRFCHWINTRIN_H=1` for `_m_prefetch` on Windows MSVC) — same mechanism category: pre-satisfy a header's include guard so its inline body never declares names that collide with libclang's builtin table. |
| 9 | **`-U__has_builtin` flag — correct mechanism documented** | Originally added during Stage 1 Task 3.5 iteration with the comment "neutralises GCC intrinsic header guards". That comment is **wrong** — GCC 11's `xmmintrin.h` has no `__has_builtin` guards. Removed as "redundant" mid-spike. Then peer evidence surfaced ppy/SDL3-CS uses the same flag in `SDL_stdinc.rsp` deliberately; flag restored with corrected comment. | Actual mechanism: `SDL_stdinc.h:127-131` defines `_SDL_HAS_BUILTIN(x)` based on `#ifdef __has_builtin`. `-U__has_builtin` forces that to 0 → `_SDL_HAS_BUILTIN(...)` always 0 → `SDL_stdinc.h:822, 853` `#if _SDL_HAS_BUILTIN(__builtin_mul_overflow)` / `__builtin_add_overflow` branches skipped → `_SDL_size_*_overflow_builtin` SDL_FORCE_INLINE helpers never declared. Defense-in-depth complement to friction #15's AST-level inline filter. Side-effect audit: 3 sites in SDL2 headers reference `_SDL_HAS_BUILTIN` — SDL_stdinc.h:822/853 (declaration gate, desired effect), SDL_assert.h:54 (`__builtin_debugtrap` choice — inline body content, no declaration impact), SDL_endian.h:134/136/138 (`HAS_BUILTIN_BSWAP{16,32,64}` macros — inline body content, no declaration impact). **No side effect on public-API declarations.** |
| 10 | **CppAst's default `TargetSystem = "windows"` on Linux host** | `CppParserOptions` ctor defaults `TargetSystem = "windows"` regardless of host OS (`src/CppAst/CppParserOptions.cs`). Triple becomes `x86_64-pc-windows-` inside the Linux container; `SDL_stdinc.h:357` `_MSC_VER` branch activates; `#include <sal.h>` fails. | Explicit `TargetSystem = "linux"` override. Strategy-brief rule "no `--target` cross-compile flag" reads as "don't target a platform that isn't the host," not "leave TargetSystem default" — being explicit about reality is part of that discipline. §7's spike ran on a Windows host where the default was incidentally correct; this only surfaces in container Linux runs. |
| 11 | **NuGet `libclang.runtime.linux-x64` ships no clang resource directory** | Stand-alone `libclang.so` (verified by enumerating the 20.1.2 package contents) — no `lib/clang/<ver>/include/` directory. libclang has no built-in path to compiler-shipped headers (`stddef.h`, `stdint.h`, `stdarg.h`) or intrinsic headers. Documented stance: `dotnet/ClangSharp#414` (Tanner Gooding) — "ClangSharp defers to libClang and does not introduce special state — users must explicitly pass all desired include directories." | Container provides real GCC + glibc via apt; Dockerfile sets `CPATH="/usr/lib/gcc/x86_64-linux-gnu/11/include:/usr/include/x86_64-linux-gnu:/usr/include"`. libclang reads `CPATH` natively, same mechanism as the clang CLI. Peer reference: ppy/SDL3-CS uses `CPLUS_INCLUDE_PATH` in their Dockerfile entrypoint with the same intent. |
| 12 | **SDL2 platform-specific system headers not available on Linux** | `<process.h>` (Win32 `_beginthreadex` in `SDL_thread.h`), `<windows.h>` (HWND/HDC/HINSTANCE in `SDL_syswm.h` / `SDL_egl.h` / `SDL_opengl.h`), `<Inspectable.h>` (WinRT C++/CX `IInspectable` in `SDL_syswm.h`), `<AvailabilityMacros.h>` and `<TargetConditionals.h>` (Apple toolchain in `SDL_platform.h`) — none exist inside the Linux container; parse fails under Windows-flavour / Apple-flavour views. | `build/_build/Targets/GenerateBindings/SyntheticHeaders/` — 5 mandatory + 3 defensive stub headers. Mandatory: `process.h`, `windows.h` (with opaque `typedef void* HWND` aliases for syswm/egl/opengl signatures), `Inspectable.h` (with `typedef struct _IInspectable IInspectable;`), `AvailabilityMacros.h`, `TargetConditionals.h`. Defensive: `winapifamily.h`, `directfb.h`, `os2.h` (driver macros never enabled in any view; cheap insurance). No business logic — pure type stand-ins. Peer reference: ppy/SDL3-CS ships exactly one such stub (`process.h`, ~80 bytes) for non-Windows hosts; we ship more because Stage 1 covers eight parse views vs ppy's narrower matrix. **This is a permanent feature of the Linux-canonical single-host strategy, not a workaround**; retirement only on escalation to multi-OS-runner pipeline. |
| 13 | **`/usr/include/EGL/egl.h` uses `__declspec` under Windows-flavour views** | `EGL/egl.h:150-166` declares functions with `EGLAPI` macro that expands to `__declspec(dllimport/export)` when a Windows target macro is active. `SDL_egl.h` pulls it under `SDL_VIDEO_DRIVER_WINDOWS`. clang's default C mode rejects `__declspec` syntax. | Add `-fdeclspec` to parser additional arguments. Narrower than `-fms-extensions` (which enables the full MS dialect); just allows `__declspec` keyword parsing. |
| 14 | **Apple deployment-target precondition checks in `SDL_platform.h`** | `SDL_platform.h:117-119` errors out with "SDL for Mac OS X only supports deploying on 10.7 and above" when `MAC_OS_X_VERSION_MIN_REQUIRED < 1070`. Under MacOS view, the macro is undefined (Apple SDK absent), check fails. | MacOS view defines `MAC_OS_X_VERSION_MIN_REQUIRED=1070`. iOS view defines `TARGET_OS_IPHONE=1` (takes SDL_platform.h's iOS branch which self-defines `__IPHONEOS__` and skips the macOS version check entirely). |
| 15 | **Translator filter is path-based only — SDL_FORCE_INLINE helpers leak as `[DllImport]` entries** | 16 SDL_FORCE_INLINE distinct names emitted into our Neutral output: `SDL_RectEmpty`, `SDL_RectEquals`, `SDL_PointInRect`, `SDL_FRectEmpty`, `SDL_FRectEquals`, `SDL_FRectEqualsEpsilon`, `SDL_PointInFRect`, `SDL_memset4`, `SDL_memcpy4`, `SDL_size_add_overflow`, `SDL_size_mul_overflow`, `SDL_MostSignificantBitIndex32`, `SDL_HasExactlyOneBitSet32`, `SDL_SwapFloat`, `_SDL_size_add_overflow_builtin`, `_SDL_size_mul_overflow_builtin`. SDL2 shared library does NOT export these — calling them would raise `EntryPointNotFoundException`. SDL_Swap16/32/64 escape this only because MSVC `#define`s shadow them as macros (the parser sees a macro, not a function declaration). | Peer state-of-the-art for filtering: Silk.NET (`src/Core/Silk.NET.BuildTools/Cpp/Clang.cs:1347-1358`) hard-skips `FunctionDecl.IsInlined` cursors during scrape; ClangSharpPInvokeGenerator (used by ppy/SDL3-CS) transpiles inline functions into managed C# bodies instead of P/Invokes; `amerkoleci/Alimer.Bindings.SDL` — our closest CppAst analog — does **not** filter and emits the same kind of bogus `[LibraryImport]` entries we surfaced (confirmed bug in their Generated/Commands.cs). For our CppAst pipeline the analog filter is `function.Flags.HasFlag(CppFunctionFlags.Inline)` in `CppAstToPreviewModel`. Defense-in-depth pair: §11.2 dynapi cross-check. |

### 11.2 Defense-in-Depth — Dynapi Manifest Cross-Check

SDL2's source tree ships `src/dynapi/SDL2.exports` — a Watcom-format manifest enumerating every public export. Generated by `gendynapi.pl`. Lines look like:

```text
++'_SDL_Init'.'SDL2.dll'.'SDL_Init'
++'_SDL_RenderGeometry'.'SDL2.dll'.'SDL_RenderGeometry'
# ++'_SDL_iPhoneSetAnimationCallback'.'SDL2.dll'.'SDL_iPhoneSetAnimationCallback'    # Watcom-build conditional
```

Parse regex: `^\+\+'_(\w+)'\.'SDL2\.dll'\.'\w+'$` captures the public-API name. Comment lines (`#`) are Watcom-build conditional, not platform-conditional — SDL2's dynapi design ships the same C function-pointer table across Linux / macOS / Windows; the `.so` / `.dylib` / `.dll` all export the same symbol set. Single cross-platform ground truth.

Cross-check semantics:
- **Generator output ∩ manifest** = correct public API entries.
- **Generator output \ manifest** = false-positive (sızıntı; static inline helper, internal symbol, etc.). Fail-closed.
- **Manifest \ generator output** = false-negative (under-emit; header exclusion too aggressive or parse view incomplete). Fail-closed.

Peer evidence: ppy/SDL3-CS `generate_bindings.py::check_generated_functions` cross-checks generated symbols against `sdl.json` (output of SDL3's `gendynapi.py --dump`). Nobody uses `nm` / `dumpbin` / `otool` because SDL itself ships the textual manifest. This is the only peer-validated "binary export oracle" pattern and it is free.

Defense-in-depth deployment pattern (matches the build host's PreFlight validator reuse model — a single validator runs in multiple stages with different fail-closed thresholds):
- **Stage 1 Task 3.5 / GenerateBindings target** — post-emit verification immediately after the parse-loop runs, fail-closed on false-positives, log false-negatives as warnings to support iterative header exclusion tuning.
- **Stage 2 PreFlight** — coherence check before native build work.
- **Stage 2 Pack stage** — `BindingSymbolExistenceValidator` (already on the plan as a deferred Stage 2 deliverable) runs the same validator against packaged native artefacts.

Same validator, three stages. Cross-stage reuse mirrors the existing build host's HybridStaticOverlayValidator + G54 alignment validator patterns.
...
### 11.4 Output Verification Snapshot (2026-05-16, first successful smoke)

After all 9 frictions resolved end-to-end:

- **Header set:** 88 headers in `vcpkg_installed/x64-linux-hybrid/include/SDL2/` minus exclusions (umbrella `SDL.h`, scaffolding `begin_code.h` / `close_code.h`, satellite umbrellas `SDL_image.h` / `SDL_mixer.h` / `SDL_net.h` / `SDL_ttf.h`, `SDL2_*` gfx prefix, GL/EGL convenience wrappers with zero SDL_ functions `SDL_opengl.h` / `SDL_opengles.h` / `SDL_opengles2.h` / `SDL_egl.h`, GL sub-headers, test scaffolding `SDL_test*.h` prefix) → **54-58 headers** parsed per view (depending on view-specific transitive resolution).
- **Function counts:** Neutral 848 (including 16 SDL_FORCE_INLINE leaks — to be filtered per friction #15) / WindowsDesktop +8 platform-only / WinRT +12 / GDK +12 / Linux +2 / MacOS 0 / IOS +4 / Android +13. Total cross-view union ≈ 899 entries.
- **Top source headers** (Neutral): SDL_stdinc.h 132, SDL_video.h 97, SDL_render.h 81, SDL_joystick.h 58, SDL_gamecontroller.h 58.
- **No satellite leakage**: zero `IMG_*`/`Mix_*`/`TTF_*`/`SDLNet_*`/`SDL2_*` names emitted across all eight views — exclusion list works.
- **vs. SDL2-CS (506 distinct externs):** 307 gap, audited by category: ~141 scope-skip (SDL2-CS deliberately omits SDL_stdinc / SDL_atomic / SDL_assert because .NET has managed equivalents — `string.Length`, `Math.Sin`, `Array.Sort`, `Encoding.UTF8`); ~16 unintended emit (inline leaks above, eliminated by friction #15 filter); ~150 version-delta (SDL2-CS pinned to `SDL_PATCHLEVEL=22` i.e. SDL 2.0.22 of Apr 2022; our pin SDL 2.32.10 spans 6 upstream feature releases).
- **Wall time:** ~30 min cold (vcpkg binary cache hit, libclang TU spinning across 8 views × ~55 headers = ~440 TU invocations). Warm-cache subsequent runs comparable.

### 11.5 Open Items Before Stage 1 Task 4 / Stage 2

These items are tracked in the Stage 1 plan and design spec — not parked. Categories ("Stage 1 Task 4 prerequisite" / "Stage 2 scope") are recorded in those canonical docs.

- AST inline filter (`CppFunctionFlags.Inline` skip) — Silk.NET pattern.
- `-U__has_builtin` restoration with documented mechanism — defense-in-depth complement to AST filter.
- Dynapi cross-check validator implementation — Stage 1 GenerateBindings post-emit + cross-stage reuse.
- Marshalling shape parity vs SDL2-CS (`IntPtr` raw pointers vs SDL2-CS's `[In] SDL_Vertex[]` typed managed arrays) — Stage 1 Task 5 emitter scope (`CsCommandEmitter` + friendly overloads per ADR-004).
- Multi-platform binary cross-check (Win+macOS dumpbin/otool) — Stage 2 Pack-stage `BindingSymbolExistenceValidator` per existing plan.
