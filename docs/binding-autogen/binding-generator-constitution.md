...
5. **Production per-family header lists** — one complete header set per active family, currently represented by `family-config.json` `families.<family>.headers[]` in the ClangSharp spike. Bootstrap/header-subset lists are not production inputs.
...
8. **Orchestrator code** — `generate_bindings.py` (config loader, `selected_families`, pipeline order). Per-family facts (`headers`, `platform_sensitive_headers`, namespace, owner_mode) are sourced from `family-config.json`.
...
- **No conditional behavior on host OS** at generation time except via the explicit platform-view pass (`family-config.json` `platform_sensitive_headers` + ClangSharp `--define-macro`/`--undefine-macro` semantics). Synthetic platform header shims (`shims/platform-headers/`) are spike-only iteration aids for Windows-local generation; production generation runs against native platform headers via the binding-generator docker container or per-RID CI.
...
| OpenGL / EGL / GLES | `EGL*`, `GL*` (Khronos types) | `SDL_opengl*.h`, `SDL_egl.h` | **Not in scope** — entire header set excluded from parse via `family-config.json`. `SDL_GLContext` is SDL-owned (`typedef void *` in `SDL_video.h:221`), not foreign |
