...
| `Directory.Packages.props` | CPM already carries spike-side `CppAst 0.24.0` + win-x64 runtime pins for `tools/binding-spike/cppast/` | **Add Linux-canonical production pins to the Cake host:** `build/_build/Build.csproj` references `CppAst 0.24.0` + `libclang.runtime.linux-x64 20.1.2` + `libClangSharp.runtime.linux-x64 20.1.2` only. Repository-wide CPM may retain non-Linux runtime pins for tracked spike artifacts; production generation remains linux-x64 only. |
