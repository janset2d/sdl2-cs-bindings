...
**4. The pure-vs-Cake split is preserved.** AGENTS.md §"Pure code stays pure", §"Cake nativeness is a hard rule at build boundaries", and ADR-003 all apply. Parser/model/emitter policy can remain target-local and Cake-free. Header-set discovery/fingerprinting is target-local generation-input behavior, similar to Harvest's target services. `.generated-stamp` persistence is a file-backed JSON contract, so its model and repository live under `Data/BindingGeneration/` and use Cake-native filesystem/JSON helpers. The Cake-aware shell (`GenerateBindingsTask`, `BindingGenerationRunner`, `ServiceCollectionExtensions`) consumes those services/repositories and owns orchestration/logging/exception translation. This split follows the existing build-host pattern: task orchestration in `Targets/`, file contracts in `Data/`, cross-cutting validation in `Validation/`.
...
build/_build/Data/BindingGeneration/              ← file-backed/tool-read binding contracts
├── GeneratedStamp.cs
└── GeneratedStampRepository.cs                   ← .generated-stamp reader/writer (deterministic, no wall-clock fields)
...
**Cake-native target with pure policy collaborators.** Parser/model/emitter policy stays pure where it does not cross a build-host boundary. Header discovery, header fingerprint reads, generated-file persistence, and `.generated-stamp` JSON writes are build-host I/O and therefore use `ICakeContext`, `Cake.Core.IO` paths, `CakeFileSystemExtensions`, and `CakeJsonExtensions`. The `GenerateBindingsTask`, `BindingGenerationRunner`, and `ServiceCollectionExtensions` consume Cake context, log through Cake, translate exceptions to `CakeException`, and stage outputs through Cake-native IO. This split honors [`AGENTS.md`](../../AGENTS.md) §"Pure code stays pure" and §"Cake nativeness is a hard rule at build boundaries" simultaneously.
...
- **Our own historical CppAst platform-pass spike** (summarized in [`research/binding-autogen-spike-findings.md`](research/binding-autogen-spike-findings.md); physical artifacts retired 2026-05-19) used the same pattern — `Defines = ["_WIN32=1", "WIN32=1", "__WIN32__=1", "__WINDOWS__=1"]` plus `Undefines = ["linux", "__linux", "__linux__", "__LINUX__", "__APPLE__", "__MACOSX__"]` for the Windows pass, mirror-image for Linux and macOS — and parsed `SDL_system.h` + `SDL_main.h` successfully across three platforms (9 neutral + 8 Windows + 2 Linux functions, runtime-validated). The spike used 10 C-stdlib shim headers (`stdint.h`, `stddef.h`, `ctype.h`, etc.) only because it ran on a Windows host targeting Linux; **inside the Linux container those shims fall away** — the apt sysroot provides every C stdlib header natively.
...
**2026-05-16 — Stage 1 Task 3.5 refinements (per [`research/binding-autogen-spike-findings.md`](research/binding-autogen-spike-findings.md) §11):**

- **Per-pass isolation needs a per-header inner loop.** The pass model above is correct at the parse-view granularity. Stage 1 Task 3.5's container run discovered that within a single pass, `CppParser.ParseFiles(all_88_headers, options)` collapses to ONE libclang translation unit and the GCC intrinsic chain pulled by `SDL_cpuinfo.h` overruns libclang's builtin-function table. The correct shape is **two-tier isolation**: outer loop over `PlatformCatalog` parse views, inner loop calling `CppParser.ParseFile(h, options)` per SDL2 header. This matches the peer state-of-the-art (`amerkoleci/Alimer.Bindings.SDL` for CppAst, `ppy/SDL3-CS` for ClangSharpPInvokeGenerator). CppAst's `ParseFile` internally delegates to `ParseFiles([h])` — isolation comes from the **loop**, not the API choice.
- **Stub-header retraction itself walked back, partially.** The retracted "mingw-w64 / Apple SDK stubs" claim above is still correct in spirit (no real Windows SDK, no real Xcode SDK). But Stage 1 Task 3.5 surfaced that a **narrow subset of platform-specific system headers does need shims** on the Linux container — `<process.h>` (Win32 thread API pulled by `SDL_thread.h`), `<windows.h>` (Win32 handles pulled by `SDL_syswm.h` / `SDL_egl.h` / `SDL_opengl.h`), `<Inspectable.h>` (WinRT C++/CX, pulled by `SDL_syswm.h`), `<AvailabilityMacros.h>` / `<TargetConditionals.h>` (Apple toolchain, pulled by `SDL_platform.h`). These are not present on Linux even with apt-installed GCC + glibc. **5 mandatory + 3 defensive stubs** live under `build/_build/Targets/GenerateBindings/SyntheticHeaders/`, each one a pure type stand-in (`typedef void* HWND;`, `typedef struct _IInspectable IInspectable;`, etc.) with zero business logic. Peer parity: ppy/SDL3-CS ships exactly one such stub (`process.h`, ~80 bytes); we ship more because Stage 1 covers eight parse views (vs ppy's narrower matrix). **This is a permanent feature of the Linux-canonical single-host strategy**, retired only on escalation to the multi-runner pipeline. The C-stdlib shim retraction from the historical cppast-platform spike stands — CPATH inside the container resolves real GCC + glibc headers; only the platform-specific subset above needs shim treatment.
- **Parser flags add `-fdeclspec` and `SDL_DISABLE_{IMMINTRIN,MMINTRIN,XMMINTRIN,EMMINTRIN,PMMINTRIN,MM3DNOW,LSX,LASX,ARM_NEON}_H` defines.** `-fdeclspec` lets clang's C mode accept `__declspec` syntax that the apt-installed libegl-dev header `<EGL/egl.h>` uses under Windows-flavour views. The `SDL_DISABLE_*` family are SDL2's own documented escape hatches at `SDL_cpuinfo.h:118-133` — they short-circuit the GCC intrinsic include chain that otherwise collides with libclang's builtin-function table. Treat both as parse-time defense.
- **`-U__has_builtin` flag — restored with correct mechanism.** Originally retired during Stage 1 Task 3.5 iteration as "redundant"; restored after peer evidence (ppy/SDL3-CS `SDL_stdinc.rsp`) clarified the real effect: `SDL_stdinc.h:127-131` defines `_SDL_HAS_BUILTIN(x)` via `#ifdef __has_builtin`, and undefining the macro forces `_SDL_size_*_overflow_builtin` SDL_FORCE_INLINE helpers (declaration-gated at SDL_stdinc.h:822, 853) out of the AST at parse time. Side-effect audit on the three SDL2 headers that reference `_SDL_HAS_BUILTIN` (SDL_stdinc.h, SDL_assert.h, SDL_endian.h) found zero impact on public-API declarations — only inline body content for `__builtin_debugtrap` / `__builtin_bswap*` selection, which the AST filter discards anyway.

The Stage 1 plan's `PlatformCatalog` row count (8) and macro composition match the catalog table above; the per-header inner loop and the supporting parser flags + synthetic-header set + dynapi cross-check guardrail shipped via Tasks 3.5 + Task 11.5 (commits `0db0e31` + `9a5f59e`). The remaining Stage 1 work (real binding model + per-category emitters + manifest-driven per-family config) is tracked in [`../superpowers/plans/2026-05-17-binding-generator-unified-plan.md`](../superpowers/plans/2026-05-17-binding-generator-unified-plan.md) per [`../superpowers/specs/2026-05-16-binding-generator-unified-design.md`](../superpowers/specs/2026-05-16-binding-generator-unified-design.md). Historical context: [`../superpowers/plans/superseded/2026-05-14-sdl2-core-binding-generator-stage-1.md`](../superpowers/plans/superseded/2026-05-14-sdl2-core-binding-generator-stage-1.md) + [`../superpowers/plans/superseded/2026-05-15-binding-generator-local-output-loop.md`](../superpowers/plans/superseded/2026-05-15-binding-generator-local-output-loop.md).
...
**2026-05-16 — Defense-in-depth dynapi cross-check (per [`research/binding-autogen-spike-findings.md`](research/binding-autogen-spike-findings.md) §11.2):** Stage 1 Task 3.5 added a third checkpoint that runs *immediately after AST emission* using SDL2's own textual dynapi manifest (`SDL2/src/dynapi/SDL2.exports`, autogenerated by `gendynapi.pl`) as the public-API oracle. The manifest enumerates every cross-platform export with the Watcom-format `++'_FunctionName'.'SDL2.dll'.'FunctionName'` shape (parse regex `^\+\+'_(\w+)'\.'SDL2\.dll'\.'\w+'$`); SDL2's dynapi design ships the same function-pointer table across Linux / macOS / Windows so a single text file is cross-platform ground truth. Peer precedent: ppy/SDL3-CS uses `gendynapi.py --dump`'s `sdl.json` output the same way in their `check_generated_functions` post-emit step. No peer uses `nm`/`dumpbin`/`otool` because SDL itself ships the textual manifest. The three-stage deployment now reads: **Stage 1 GenerateBindings target** (post-emit text-manifest cross-check, catches AST-filter leaks and over-aggressive header exclusions before they ship in `.generated-stamp`) → **Stage 2 PreFlight** (vcpkg-state coherence) → **Stage 2 Pack** (`BindingSymbolExistenceValidator` runs binary `dumpbin`/`nm`/`otool` checks against per-RID natives). Same `SDL2.exports` oracle file can be reused by the binary-stage validator as the expected-set input, removing the need for the binary-stage validator to enumerate exports independently.

**Wire-format coverage is a known gap across all three checkpoints (post-2026-05-16 Stage 1 Task 3.5 review).** The dynapi manifest and the binary symbol tables are both **name-only** dictionaries — Watcom DEF-files carry no type information; PE/ELF/Mach-O dynamic export tables are byname dispatch maps that do not record C signatures. As a result the three validators above can prove "the function is named correctly + exists in the native library" but cannot prove "the function's parameter widths, parameter order, and return-type bytes match the C ABI." The Stage 1 Task 3.5 production smoke (2026-05-16) shipped 866 name-correct emits with bugs in `MapTypedef` and `MapPrimitive` causing wire-format ABI corruption for several SDL_* primitives and Linux `long` — name-validator gate stayed green throughout. The Stage 2 Pack-stage validator inherits the same limitation and **elevates the importance** of per-RID consumer-smoke matrix coverage (already in `release.yml`) as the only realistic place to catch wire-format mismatch — by actually calling generated P/Invokes against the native library on each RID and observing whether marshalling round-trips. Candidate fourth-layer guardrails for the signature-wire gap (none chosen, all parked as future Phase 4 hardening):

- (a) Static C-side cross-check via an independent parser of SDL2 public headers producing signature tuples for diff against the generator's emit (cost: a second parser to maintain — defeats CppAst single-source advantage).
- (b) Per-RID consumer smoke matrix invoking selected functions with sentinel arguments and asserting return values match expected (hand-curated function-spot list; higher coverage = higher curation cost; natural home is the existing `consumer-smoke` matrix in `release.yml`).
- (c) Runtime `Marshal.PrelinkAll` symbol-presence check — same coverage as dynapi/binary-table, no signature gain.

The superseded Stage 1 plan's post-implementation review section ("[`../superpowers/plans/superseded/2026-05-14-sdl2-core-binding-generator-stage-1.md`](../superpowers/plans/superseded/2026-05-14-sdl2-core-binding-generator-stage-1.md) §Post-Implementation Review Findings 2026-05-16 / Validator wire-format coverage gap") tracks this gap and the candidate layers; resolution is folded into the unified plan [`../superpowers/plans/2026-05-17-binding-generator-unified-plan.md`](../superpowers/plans/2026-05-17-binding-generator-unified-plan.md) Phase 3 acceptance gates.
...
| `build/_build/Targets/GenerateBindings/` | Partially scaffolded in Stage 1 execution | **Complete** the Cake target home per [`AGENTS.md`](../../AGENTS.md) §Build-Host Reference Pattern + ADR-002. Hosts orchestration (`GenerateBindingsTask`, `BindingGenerationRunner`, `ServiceCollectionExtensions`) plus target-local collaborators. Cake-native boundaries own header IO, generated-file IO, and stamp JSON; pure parser/model/emitter policies stay Cake-free where no IO boundary is crossed. |
| `build/_build/Validation/BindingGeneration/` (vcpkg-state coherence) | Binding-specific validators not yet present | **Add** validator joining existing PreFlight validators (`HybridStaticOverlayValidator`, manifest schema validators). Behavior-first name candidate: `BindingVcpkgCoherenceValidator`. Reads `.generated-stamp` per family + current vcpkg state; fails PreFlight with actionable error on drift. |
| `build/_build/Validation/BindingGeneration/` (symbol existence) | Binding-specific validators not yet present | **Add** validator running at Pack stage (after Harvest, before Package). Behavior-first name candidate: `BindingSymbolExistenceValidator`. Cross-platform via existing `build/_build/Tools/` wrappers around `dumpbin /exports` / `nm -D --defined-only` / `nm -gU`. |
...
| `Directory.Packages.props` | CPM carries the production CppAst 0.24.0 + linux-x64 runtime pins used by `build/_build/Build.csproj`; historical spike-side win-x64 pins are no longer needed after the spike artifacts were retired. | **Keep production generation Linux-canonical:** `build/_build/Build.csproj` references `CppAst 0.24.0` + `libclang.runtime.linux-x64 20.1.2` + `libClangSharp.runtime.linux-x64 20.1.2` only. Remove stale non-Linux runtime pins in a dedicated package-management cleanup if they remain in CPM. |
...
| `docs/decisions/2026-05-14-binding-autogen-toolchain.md` | Exists | **Maintain** as the companion ADR recording the toolchain decision (CppAst lock + version-trio pin + scope-trajectory rationale + migration door). |
...
| **Error 4 — toolchain stub strategy speculation (corrected 2026-05-15)** | The 2026-05-14 draft claimed Stage 1 would use "MinGW headers or stubs from the container's apt layer" for the Windows pass and "minimal Apple SDK header stubs" for the macOS pass. Both were speculation, not evidence-grounded. WebFetch of ppy/SDL3-CS (Dockerfile + `generate_bindings.py`) on 2026-05-15 confirmed ppy uses **preprocessor-macro switching only** — no mingw-w64, no Apple SDK, one tiny `include/process.h` shim (81 bytes). Re-reading our historical CppAst platform-pass spike confirmed the same pattern — `Defines`/`Undefines` macro juggling, no `--target`, no platform-specific header stubs (the spike's 10 stubs are pure C-stdlib shims for cross-host parsing and disappear inside the Linux container). The physical spike artifacts were retired on 2026-05-19, but the evidence remains summarized in [`research/binding-autogen-spike-findings.md`](research/binding-autogen-spike-findings.md). Per-OS macro groups in [`SDL_syswm.h`](../../vcpkg_installed/x64-windows-hybrid/include/SDL2/SDL_syswm.h) (lines 86, 94-95, 105-106, 109-110, 120) carry their own forward declarations of `NSWindow`, `UIWindow`, `ANativeWindow`, `gbm_device`, etc., so function-level platform surface parses with zero hand-written stubs. | The strategy section "Multi-pass parsing strategy" has been rewritten to reflect the preprocessor-macro-only approach. Stage 1 scope expanded to cover the full 7–8 entry `PlatformCatalog` at function level; `SDL_syswm.h` typed-union layout was the only piece that legitimately needs forward-declaration stubs (~15–20 types), and that work is now scoped to Stage 2. **Partial retraction below — see Error 5.** |
| **Error 5 — Stage 1 Task 3.5 production refinements (recorded 2026-05-16)** | Three corrections from the Stage 1 Task 3.5 container run, against earlier claims in this brief: **(a)** Per-pass macro switching alone is **insufficient**; per-header `CppParser.ParseFile` inner loop is required to isolate libclang TU pollution from the SDL_cpuinfo.h GCC-intrinsic chain. The §"Multi-pass parsing strategy" pattern is now two-tier (outer view loop + inner header loop), matching the peer state-of-the-art (Alimer for CppAst, ppy for ClangSharp). **(b)** Error 4's "zero hand-written stubs" claim is **partially walked back**. C-stdlib shims (stddef/stdint/stdarg) are genuinely unnecessary (CPATH resolves real GCC + glibc), but a narrow subset of platform-specific system headers (`<process.h>`, `<windows.h>`, `<Inspectable.h>`, `<AvailabilityMacros.h>`, `<TargetConditionals.h>`) **does** require shim treatment. 5 mandatory + 3 defensive stubs at `build/_build/Targets/GenerateBindings/SyntheticHeaders/` — pure type stand-ins with zero business logic. Peer parity: ppy/SDL3-CS ships exactly one such stub (`process.h`); we ship more because Stage 1 covers eight parse views. **(c)** The Stage 1 Task 3.5 mid-iteration claim "`-U__has_builtin` is gratuitous, drop it" was wrong. The flag's correct mechanism — neutralizing `_SDL_HAS_BUILTIN(x)` so SDL_stdinc.h:822, 853 `_SDL_size_*_overflow_builtin` SDL_FORCE_INLINE helpers never enter the AST — was clarified by peer evidence (ppy/SDL3-CS `SDL_stdinc.rsp`). Side-effect audit on the three SDL2 sites that reference `_SDL_HAS_BUILTIN` found zero impact on public-API declarations. **Plus**: post-emit dynapi-manifest cross-check (`SDL2/src/dynapi/SDL2.exports`) introduced as a third defense-in-depth checkpoint — see updated §"Symbol-existence validation guardrail" above. | Stage 1 implementation is honest about the asymmetry: per-view macro switching does not by itself produce a clean AST; the inner per-header loop is structural, not optional. Future contributors reading the brief in 2027 should understand the platform-stubs subset is durable maintenance surface (mirrored in `docs/playbook/binding-generator-maintenance.md`), not a workaround. The dynapi-manifest oracle is now the single source of public-API truth used by three validators across the pipeline. |
| **Reframe — generator hosted in Cake build host (2026-05-15)** | The 2026-05-14 draft proposed a standalone `src/Janset.SDL2.Bindings.Generator/` console app invoked from the Cake target via `Tool<TSettings>` wrapper. Per [`AGENTS.md`](../../AGENTS.md) §Build-Host Reference Pattern + ADR-002 + ADR-003 review, binding generation is unambiguously a CI/CD concern and the Cake build host already owns equivalent infrastructure (vcpkg state, manifest parsing, validation, tool wrappers, logging, paths, runtime profile). A parallel `src/`-tree orchestration stack would duplicate that surface and break ADR-002 §2.1's target-centric navigation rule. | The brief's HOW section was rewritten to fold the generator into `build/_build/Targets/GenerateBindings/`. Follow-up correction clarifies the boundary: parser/model/emitter policy can stay Cake-free, but header/stamp/generated-file I/O remains Cake-native and testable through `FakeCakeWorld`. |
