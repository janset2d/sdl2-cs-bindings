...
- ~~**Vcpkg package-info provider test migration** (A7+E8)~~ — closed after the package-info provider was absorbed into `Build.Tools.Vcpkg`: the old provider tests were deleted, typed parser/alias tests use `FakeCakeWorldV2.WithProcessResult`, and the fixture now initializes redirected stdout/stderr streams consistently.
