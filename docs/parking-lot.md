...
### Immediate Next-Step Priority — VcpkgManifestReader → Repository absorb (P10 kickoff)

- Status: `priority-immediate` (next slice's first task)
- Surfaced post-S15 P9 Repository pattern correction (2026-05-10). `IVcpkgManifestReader` is currently exposed as a public DI seam under `Build.Vcpkg/` and inject-passed into `VcpkgManifestRepository(ICakeContext, IVcpkgManifestReader, FilePath)`. The Reader is a low-level JSON parser; the Repository is the file-backed facade. The Reader has exactly one production consumer (the Repository itself), so the public seam earns nothing — it's pre-S15 wiring carried through P9 without revisiting.
- **Action**: Absorb `VcpkgManifestReader` into `VcpkgManifestRepository`. Either inline the parse into `Repository.Load*` methods, or keep the Reader as `internal sealed` inside `Build.Repositories` (private-helper pattern). Strip `IVcpkgManifestReader` interface entirely — single consumer, no test seam needed once Reader is internal. Strip the `services.AddSingleton<IVcpkgManifestReader, VcpkgManifestReader>();` registration from `Vcpkg/ServiceCollectionExtensions.cs.AddVcpkg()`.
- **Files touched**: `Vcpkg/{IVcpkgManifestReader,VcpkgManifestReader,ServiceCollectionExtensions}.cs`; `Repositories/{VcpkgManifestRepository,ServiceCollectionExtensions}.cs`; `Tests/Unit/Vcpkg/VcpkgManifestReaderTests.cs` (relocate or fold into VcpkgManifestRepositoryTests). Pure mechanical refactor; no behavior change.
- **Why immediate**: P9 closure noted Vcpkg promotion to root `Build.Vcpkg/` named concept (cross-target consumer = Harvest + PreFlight). Keeping a public Reader seam alongside the Repository duplicates the entry point — readers see two ways to load vcpkg manifests when there should be one. P10 should land this as its first task before any other relocation work.
...
- **Catch-all `Exception` narrowing** (C4+C5+C6, pre-existing): `BinaryClosureWalker.BuildClosureAsync` + `VcpkgCliProvider.GetPackageInfoAsync` both use `#pragma warning disable CA1031` + `catch (Exception ex)` to convert every error into a typed Result. Masks programmer bugs (NRE, OOM, contract violations). File-scope comment in BinaryClosureWalker explicitly defers ("Narrowing to operational types … changes pre-migration behavior; deferred to a follow-up slice"). Tracked here so the deferral surfaces.
...
- **VcpkgCliProviderTests V2 migration** (A7+E8): The relocated file `Unit/Vcpkg/VcpkgCliProviderTests.cs` (NEW location; touched in P9) keeps V1 `FakeCakeToolContextBuilder` + raw `FakeFileSystem`/`FakeEnvironment` + `FakeLog` + `WithStandardOutput` setup. V2 `FakeCakeWorldV2` lacks an equivalent for tool-stdout injection (`WithProcessResult`/`WithDefaultProcessResult` exist but the Cake `Tool<TSettings>` boundary is different). Touched-test rule violation deferred until V2 fixture grows the equivalent helper. New file opting out of V2 is the bigger sin per Reviewer E — flag if a sibling new test follows the same pattern.
...
- **`Host/Configuration/RepositoryConfiguration` namespace residue** (B6): Sole survivor of `Host/Configuration/`. Either fold `RepoRoot` into `BuildContext`/`IPathService` and delete the namespace, or document the exemption. P10 cleanup territory.
- **Lost regression coverage on `--project` argument shape** (E NIT): Deleted V1 `PackageConsumerSmokePipelineTests.CreateSmokeTestArguments_Should_Use_Project_Option_For_Project_Path` asserted `dotnet test --project <path>` argument shape. New V2 happy-path scenario only checks "test" + TFM appear somewhere in args — loses the regression. Add an explicit `Arguments.Contains("--project", StringComparison.Ordinal)` assertion to the happy-path scenario, or a dedicated argument-shape unit test on `DotNetSmokeRunner`.
- **Test discipline polish cluster** (E1-E7 + NIT cluster): `BuildScopeManifest` dead helper, happy-path cardinality assertions missing, `NewWorld(feedClient)` unused param, verbose `Arg.Is<T>(t => t == x)` patterns, fully-qualified `LogLevel` repeats, MonoAvailabilityProbe whitespace-PATH edge case, "When_Happy_Path" test naming convention drift, PublishPublic test missing exception-type assertion, validator separate Code+Message asserts (couple them), "3 TFMs" pluralization brittleness. Pure polish; bundle with V2 fixture pass.
- **C# code-quality SUGGEST cluster** (C7-C20 + NIT): `ShouldSkipTfm` `out string` → `(bool, string)` tuple, `SmokeScopeComparator.Descendants()` typed-query optimization, `ReadmeMappingTableBlock.BuildBlock` StringBuilder + LF/CRLF inconsistency, `BinaryClosureWalker.MatchesPattern` silent failure on multi-`*` patterns, `AuthEnvVarChain` shape (ImmutableArray vs string[]), `ResolveConcreteFamiliesInScope` O(N·M) → Dictionary lookup, `MonoAvailabilityProbe` doesn't recognize `mono.exe` on Windows (acceptable since caller gates non-Windows but probe class reads platform-agnostic), `VcpkgCliProvider.ArgumentException.ThrowIfNullOrEmpty` → `ThrowIfNullOrWhiteSpace` consistency, `PackageInfoError` could be `record` post-coupling-fix. Heterogeneous polish; address per-file as next slices touch them.
- **Stale `ArchitectureTests` cleanup** (A8): `Tests/Unit/CompositionRoot/ArchitectureTests.cs` has stale crefs to deleted namespaces (`Build.Integrations.Vcpkg.VcpkgCliProvider` line 92, `Build.Features.Packaging.ArtifactSourceResolvers` line 165). ADR-002 §14 explicitly retires architecture tests; P10 territory. Verify file still compiles green at end of S15 — if it does, defer is fine; if it would fail, fix the cref-only lines now.
