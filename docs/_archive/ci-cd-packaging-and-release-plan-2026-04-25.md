...
> **ARCHIVED 2026-04-25.** Pre-Slice-E-follow-up-pass design snapshot. Substantial sections describe the retired `Release-Candidate-Pipeline.yml` mega-workflow (superseded by `release.yml` per [ADR-003 §3.4](../decisions/2026-04-20-release-lifecycle-orchestration.md)) and the retired `prepare-native-assets-*.yml` family (deleted 2026-04-25 in P8.1). Forward-looking design fragments (`build/known-issues.json` opt-out, `force_build_strategy` 3-tier enum, `Promote-To-Public.yml` workflow, `PR-Version-Consistency-Check.yml`) are carried forward to [`docs/phases/phase-2-adaptation-plan.md`](../phases/phase-2-adaptation-plan.md) Pending Decisions for Phase 2b/3 evaluation. Canonical replacement: [`docs/knowledge-base/ci-cd-packaging-and-release-plan.md`](../knowledge-base/ci-cd-packaging-and-release-plan.md) (live-state pointer + backlog index).
