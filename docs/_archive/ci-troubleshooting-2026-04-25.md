...
> **ARCHIVED 2026-04-25.** Pre-Slice-E-follow-up-pass troubleshooting playbook. The "Workflow Overview" section describes the retired `prepare-native-assets-{main,windows,linux,macos}.yml` family (deleted 2026-04-25 in P8.1) and references workflow steps that no longer exist (release.yml + build-linux-container.yml absorb the work). Vcpkg-port-specific debug tips (autoconf 2.70+ requirement, mpg123 arm64 NEON64 + fixed-point overlay, cache key invalidation triggers, macOS Xcode/Homebrew specifics) carry forward to the canonical replacement. Canonical replacement: [`docs/playbook/ci-troubleshooting.md`](../playbook/ci-troubleshooting.md) (rewritten for `release.yml` topology).
