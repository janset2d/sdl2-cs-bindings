...
Old style (`SDL_Window*` pointer references) is rewritten to by-value at every single-pointer occurrence. Pattern B rewrite scope spans three raw-ABI positions: method parameter type, method return type, and **struct field type** (single-pointer `SDL_X*` → by-value `SDL_X`). Double-pointer (`SDL_Window**`) and `out` parameter positions are preserved. The ABI invariant holds at field positions because a Pattern B struct carries exactly one `nint` field — its layout is bit-identical to a pointer at the corresponding C struct field offset, so `SDL_SysWMmsg* msg` (C) and `SDL_SysWMmsg msg` (C#) occupy the same 8/4-byte slot without padding shift.

Cross-assembly Pattern B (satellite consuming Core handles by value in its own `[LibraryImport]` surface) requires `[assembly: DisableRuntimeMarshalling]` per Constitution §"Opaque Handles" — see the **Cross-assembly Pattern B contract** sub-clause for the SYSLIB1051 rationale, the `NET7_0_OR_GREATER` conditional, and the blittable-only assembly invariant that justifies the attribute. Every generation-emitting assembly (`Janset.SDL2.Core`, `Janset.SDL2.Image`, future satellites) lands the attribute under `Support/DisableRuntimeMarshalling.cs`.
