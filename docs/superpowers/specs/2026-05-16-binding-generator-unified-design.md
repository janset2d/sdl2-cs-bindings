...
>
> **Translation-rule supersession (2026-05-19):** This spec remains useful for generator architecture and workstream history. Detailed C-to-C# translation rules are superseded by [`../../binding-autogen/binding-translation-contract.md`](../../binding-autogen/binding-translation-contract.md). In particular, the earlier `long -> nint` claim is wrong for Windows LLP64 and must not be used as current policy.
>
...
`TypeMappingPolicy.MapPrimitive` originally proposed `Long → "nint"` as a correction for Linux LP64. That detail is superseded and known wrong for Windows LLP64; current policy is in [`../../binding-autogen/binding-translation-contract.md`](../../binding-autogen/binding-translation-contract.md). `MapTypedef` chain-resolves through nested typedefs before falling back to `SDL_`-prefix → `IntPtr` (P0.1 — `SDL_AudioFormat`/`SDL_SpinLock`/`SDL_GameControllerButton` now emit their underlying primitive width). `Map`'s catch-all branch raises a typed warning rather than silently emitting `IntPtr` (P0.3).
...
- **Visual peer-oracle comparison** — after Phase 3, manually diff representative output against (a) `external/sdl2-cs/src/SDL2.cs` (current hand-written shape, untrusted-for-production-testing but legitimate API surface oracle); (b) historical local spike findings summarized in `docs/binding-autogen/research/binding-autogen-spike-findings.md`; (c) `Alimer.Bindings.SDL` `Generated/Commands.cs` + `Handles.cs` (production CppAst SDL3 reference); (d) `ppy/SDL3-CS Generated/` (production ClangSharp SDL3 reference). Differences fall into four categories: typed-handle delta (we emit, they emit raw `IntPtr`), friendly-overload delta (we emit, peer doesn't), known SDL2-vs-SDL3 API drift, true generator defect.
