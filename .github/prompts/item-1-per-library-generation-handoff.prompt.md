...
- **S1-4 Tasks 4.2 → 4.10 are non-divisible.** Do NOT run `generate_bindings.py --family all --execute` between these tasks; intermediate state would either crash (JSON schema vs LoadRoster mismatch) or silently break cross-family pull. The Common D-block verification runs only at Task 4.14, after all rewriter changes land.
