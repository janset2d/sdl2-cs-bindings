...
namespace Build.Tests.Unit.Application.Diagnostics;
...
public sealed class InspectHarvestedDependenciesTaskRunnerTests
{
    ...
    public async Task RunAsync_Should_Throw_When_Platform_Is_Unknown()
    {
        ...
        var profile = CreateRuntimeProfile("win-x64", PlatformFamily.Unknown);
        var runner = new InspectHarvestedDependenciesTaskRunner(
    ...
    public async Task RunAsync_Should_Throw_When_Primary_Binary_Missing_From_Extracted_Payload()
    {
        ...
        var profile = CreateRuntimeProfile("win-x64", PlatformFamily.Windows);
        var runner = new InspectHarvestedDependenciesTaskRunner(
            repo.CakeContext, new FakeLog(), repo.Paths, profile, ManifestFixture.CreateTestManifestConfig());
