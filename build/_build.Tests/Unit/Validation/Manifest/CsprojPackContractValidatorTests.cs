...
public sealed class CsprojPackContractValidatorTests
{
    ...
    private static async Task AssertFails(CsprojPackContractValidation validation, CsprojPackContractCheckKind expectedKind)
    {
        await Assert.That(validation.HasErrors).IsTrue();
        var hasExpectedKind = validation.Checks.Any(check => !check.IsValid && check.Kind == expectedKind);
