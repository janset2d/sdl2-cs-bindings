...
/// <summary>
/// Coverage for the scope-contains path of CrossFamilyDependencyResolvabilityValidator [G58].
/// These tests do not exercise the optional external-feed probe states.
/// </summary>
public sealed class CrossFamilyDependencyResolvabilityValidatorTests
{
    [Test]
    public async Task Validate_Should_Return_InScope_When_Dependency_Is_In_Mapping()
    {
        ...
        var validator = new CrossFamilyDependencyResolvabilityValidator();
        ...
        await Assert.That(check.Status).IsEqualTo(CrossFamilyDependencyCheckStatus.InScope);
    ...
    public async Task Validate_Should_Return_Missing_When_Dependency_Is_Not_In_Mapping()
    {
        ...
        var versions = new PackageFamilyVersionSet(
        [
            new PackageFamilyVersion(new PackageFamilyId("sdl2-image"), NuGetVersion.Parse("2.8.1-hotfix.1")),
        ]);
        ...
        await Assert.That(validation.Checks.Count).IsEqualTo(1);
        var check = validation.Checks[0];
        await Assert.That(check.Status).IsEqualTo(CrossFamilyDependencyCheckStatus.Missing);
        await Assert.That(check.DependentFamily).IsEqualTo("sdl2-image");
        await Assert.That(check.DependencyFamily).IsEqualTo("sdl2-core");
    ...
    public async Task Validate_Should_Flag_Missing_When_Mapping_Contains_Family_Not_In_Manifest()
    {
        ...
        var versions = new PackageFamilyVersionSet(
        [
            new PackageFamilyVersion(new PackageFamilyId("sdl-ghost"), NuGetVersion.Parse("1.0.0")),
        ]);
    ...
    public async Task Validate_Should_Return_InScope_For_Chain_DependsOn_When_All_In_Mapping()
    {
        ...
        [
            new PackageFamilyVersion(new PackageFamilyId("sdl2-core"), NuGetVersion.Parse("2.32.0-local.test")),
            new PackageFamilyVersion(new PackageFamilyId("sdl2-image"), NuGetVersion.Parse("2.8.0-local.test")),
            new PackageFamilyVersion(new PackageFamilyId("sdl2-ttf"), NuGetVersion.Parse("2.24.0-local.test")),
        ]);
    ...
    public async Task Validate_Should_Flag_Multiple_Missing_When_Concurrent_Satellites_Lack_Core()
    {
        ...
        var missingChecks = validation.Checks.Where(c => c.Status == CrossFamilyDependencyCheckStatus.Missing).ToList();
