...
/// <summary>
/// Covers the fake-filesystem precondition paths for <see cref="NativeSmokePipeline"/>.
...
public sealed class NativeSmokeTaskRunnerTests
{
    ...
    public async Task RunAsync_Should_Throw_When_Harvest_Output_Missing_For_Specified_Library()
    {
        ...
        var runner = new NativeSmokePipeline(
            repo.CakeContext,
            new FakeLog(),
            repo.Paths,
            profile,
            ...
            repo.BuildContext.Options.Vcpkg);

        var ex = await Assert.That(() => runner.RunAsync(new NativeSmokeRequest("linux-x64"))).Throws<CakeException>();
    ...
    public async Task RunAsync_Should_Accept_Any_Rid_Without_Hardcoded_Cap()
    {
        ...
        var profile = CreateRuntimeProfile("win-arm64", RuntimeFamily.Windows);
        var runner = new NativeSmokePipeline(
            repo.CakeContext,
            new FakeLog(),
            repo.Paths,
        ...
        var ex = await Assert.That(() => runner.RunAsync(new NativeSmokeRequest("win-arm64"))).Throws<CakeException>();
