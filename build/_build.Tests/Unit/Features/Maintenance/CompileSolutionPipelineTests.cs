...
namespace Build.Tests.Unit.Features.Maintenance;

/// <summary>
/// CompileSolutionPipeline is a one-line delegation to Cake's <c>DotNetBuild</c> alias; a
...
public sealed class CompileSolutionTaskRunnerTests
{
    ...
    public async Task Ctor_Should_Reject_Null_CakeContext()
    {
        ...
        await Assert.That(() => new CompileSolutionPipeline(null!, log, repo.Paths, config))
    ...
    public async Task Ctor_Should_Reject_Null_Log()
    {
        ...
        await Assert.That(() => new CompileSolutionPipeline(repo.CakeContext, null!, repo.Paths, config))
            .Throws<ArgumentNullException>();
    }
    ...
    public async Task Ctor_Should_Reject_Null_PathService()
    {
        ...
        await Assert.That(() => new CompileSolutionPipeline(repo.CakeContext, new FakeLog(), null!, config))
            .Throws<ArgumentNullException>();
    }
    ...
    public async Task Ctor_Should_Reject_Null_BuildConfiguration()
    {
        ...
        await Assert.That(() => new CompileSolutionPipeline(repo.CakeContext, log, repo.Paths, null!))
            .Throws<ArgumentNullException>();
    }
}
