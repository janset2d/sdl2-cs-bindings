...
public sealed class PreflightReporter(ICakeContext cakeContext)
{
    ...
    public void ReportRunStart()
    {
        ...
        Log.Information("ℹ️ Scope: version consistency + hybrid-static overlay coherence + core identity + manifest family name invariants + csproj pack contract + upstream version alignment + cross-family dependency resolvability.");
    ...
    public void ReportCrossFamilyDependencyResolvability(CrossFamilyDependencyValidation validation)
    {
        ArgumentNullException.ThrowIfNull(validation);
        ...
        {
            Log.Information("  ℹ️ No cross-family dependencies in scope — no checks to run.");
            Log.Information("");
            Log.Information("✅ Cross-family dependency resolvability check PASSED - no dependencies to resolve");
            return;
        }
        ...
        foreach (var check in validation.Checks)
        {
            if (check.IsError)
            {
                Log.Error(
                    ...
                    check.ErrorMessage ?? "unknown failure");
            }
            else
            {
                Log.Information(
        ...
        if (validation.HasErrors)
        {
            ...
            Log.Error("❌ Pre-flight check FAILED - {0} cross-family resolvability violation(s) detected [G58]", failedCount);
