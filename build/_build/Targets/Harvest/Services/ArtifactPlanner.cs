    ...
    ManifestConfig manifestConfig) : IArtifactPlanner
{
    ...
    public async Task<Result<DeploymentPlan, ArtifactPlannerError>> CreatePlanAsync(LibraryManifest current, BinaryClosure closure, DirectoryPath outRoot, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            // Licenses are written RID-scoped under licenses/{rid}/{package}/...
            ...
            foreach (var node in closure.Nodes)
            {
                ...
                if (_profile.Family == RuntimeFamily.Windows)
                {
            ...
            if (_profile.Family != RuntimeFamily.Windows && itemsForUnixArchive.Count != 0)
            {
            ...
            var strategy = _profile.Family == RuntimeFamily.Windows
            ...
            return Result<DeploymentPlan, ArtifactPlannerError>.Success(new DeploymentPlan(actions, statistics));
        }
        ...
        catch (Exception ex)
        {
            ...
            return Result<DeploymentPlan, ArtifactPlannerError>.Failure(new ArtifactPlannerError(message));
        }
    }
