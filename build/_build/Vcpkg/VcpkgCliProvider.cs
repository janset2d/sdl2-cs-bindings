...
public sealed class VcpkgCliProvider : IPackageInfoProvider
{
    ...
    public async Task<Result<PackageInfo, PackageInfoError>> GetPackageInfoAsync(string packageName, string triplet, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            if (string.IsNullOrEmpty(vcpkgJsonOutput))
            {
                ...
                return Result<PackageInfo, PackageInfoError>.Failure(new PackageInfoError(message));
            }
            ...
            return Result<PackageInfo, PackageInfoError>.Success(
                new PackageInfo(PackageName: packageName, Triplet: triplet, OwnedFiles: ownedFiles, DeclaredDependencies: packageResult.Dependencies ?? []));
        }
        ...
        {
            return Result<PackageInfo, PackageInfoError>.Failure(new PackageInfoError($"Error building dependency closure: {ex.Message}", ex));
        }
    }
}
