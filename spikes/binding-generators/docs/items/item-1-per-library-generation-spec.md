...
- Core regen gains `[Flags]` on 5 Core enums (4 via allow-list — `SDL_Keymod`, `SDL_BlendMode`, `SDL_GLcontextFlag`, `SDL_RendererFlip`, `SDL_TextureModulate`; 1 via suffix — `SDL_RendererFlags`). `SDL_bool` does NOT gain `[Flags]` (neither suffix nor in allow-list). See §6 success criterion #1 carve-out.
...
| 4 | `dotnet run --file spikes/binding-generators/clangsharp/oracle.cs -- --family sdl2-core --family sdl2-image --family sdl2-ttf --family sdl2-mixer --family sdl2-gfx --write-report` produces a 5-family evidence report; **0 findings** across Priority C categories for sdl2-core + sdl2-image (no regression). TTF/Mixer/GFX rows show `SourceStatus.Missing` with zero counts (expected — not yet generated). | Report at `spikes/binding-generators/output/reports/oracle-evidence-clangsharp.md`. |
