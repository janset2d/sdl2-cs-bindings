...
- **Error macros:** `TTF_SetError`/`TTF_GetError` excluded (cross-family aliases to Core).
...
- **Error macros:** 4 to exclude (`Mix_SetError`, `Mix_GetError`, `Mix_ClearError`, `Mix_OutOfMemory`). Legacy compat aliases also excluded.
- **Version macros:** `SDL_MIXER_COMPILEDVERSION` should be kept/auto-emitted (same pattern as Image's `SDL_IMAGE_COMPILEDVERSION`). `SDL_MIXER_VERSION(X)` and `SDL_MIXER_VERSION_ATLEAST(X,Y,Z)` are function-like — skipped/reported.
