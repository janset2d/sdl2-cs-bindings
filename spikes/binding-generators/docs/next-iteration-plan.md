...
| **Item 5: SDL2_mixer Layer 1** | next | Add Mixer callbacks + `Mix_Music` owner-mode handle. This is the last satellite Layer 1 slice before Layer 2 public typed projection planning. |
...
## Current Evidence Snapshot — 2026-05-27 (Item 4 closure)
...
- Regeneration result: Core + Image + GFX + TTF generated and active; Mixer remains the only queued SDL2 satellite. 7-step conceptual postprocess pipeline: `platform-delta` → `strip-varargs` → `libraryimport` (Modern only) → `flags-detect` → `guid-substitute` → `clong-dispatch` → `uniform-opaque`.
...
- Oracle evidence: `dotnet run --file spikes/binding-generators/clangsharp/oracle.cs -- --family sdl2-ttf --write-report` writes [`../output/reports/oracle-evidence-clangsharp.md`](../output/reports/oracle-evidence-clangsharp.md). TTF raw ABI checks report 0 findings after the private dual-dispatch helper false-positive fix in `oracle.cs`.
- Item 4 shape evidence: `SDL_ttf.h` is the only TTF header; `Generated/{Compat,Modern}/Handles.g.cs` contains `TTF_Font` in `namespace SDL2.Ttf`; Modern `TTF_OpenFontIndex*` / `TTF_FontFaces` use `CLong`; Compat uses Win32/Unix64 helper dispatch; no-SDLCALL functions use Cdecl; `TTF_SetError`, `TTF_GetError`, `TTF_GetFontKerningSize`, `TTF_SetDirection`, and `TTF_SetScript` are absent.
- Peer visual evidence: SDL2-CS uses `IntPtr` for `TTF_Font` and ABI-wrong C `long` shortcuts (`long` / `IntPtr`); Janset intentionally diverges for typed handles and platform-correct C `long`. `Silk.NET.SDL` exists as a core SDL package, but the 2.23.0 NuGet package XML contains 0 `TTF_` / `SDL_ttf` / `TTF_Font` symbols and depends on `Ultz.Native.SDL`, not a TTF satellite native package, so it offers no SDL2_ttf peer surface for Item 4 comparison.
- Self-tests: Python self-test PASS and C# postprocess self-test PASS.
- Existing Core runtime ABI smoke remains the last recorded runtime test baseline: `dotnet test --project spikes/binding-generators/clangsharp/tests/abi-tests/AbiTests.csproj -c Release` passes `792/792` across `net462`, `net8.0`, `net9.0`, and `net10.0` on Windows x64. TTF-specific runtime smoke is deferred until a redistributable font asset and native copy strategy are in scope; full 7-RID proof remains a production CI gate.
