...
| **Next — Iteration 2: Config Surface Unification** | ⏭ next | Consolidate/standardize the 18-source config surface identified in [`satellite-expansion-roadmap.md`](satellite-expansion-roadmap.md) without touching `build/manifest.json`, preserving determinism and family isolation. |
| **Items 2–5: Image/GFX/TTF/Mixer Layer 1 completion** | ⏳ queued | Expand Layer 1 raw ABI after Iteration 2. Item 2 verifies the S1-6 `IMG_InitFlags` fix; Item 3 adds GFX; Item 4 adds TTF C `long` + `TTF_Font`; Item 5 adds Mixer callbacks + `Mix_Music`. Layer 2 public typed projection defers until all five families have stable Layer 1. |
...
- [`../clangsharp/policy/opaque-handle-roster.json`](../clangsharp/policy/opaque-handle-roster.json) — family-keyed schema 2.0 single source of truth for Pattern B handle auto-detect, force-opaque, and excluded-candidate lists.
