...
| **Next — Slice 7: Remaining satellite families Layer 1 completion** | ⏳ Active planning | Expand Layer 1 raw ABI to SDL2_ttf, SDL2_mixer, SDL2_gfx. Family RSPs, csprojs, header scope lists, include directory wiring. Per-family header deep-dive agents identify ABI risks (TTF: C `long` reuse of Slice C-A hybrid pattern; Mixer: callback delegates + `Mix_Chunk` union; GFX: per-header `SDL2_*_SCOPE` export macros + mixed naming surface). Layer 2 public typed projection defers until all five families have stable Layer 1. Detailed approach in [`ppy-reference-analysis-2026-05-25.md`](ppy-reference-analysis-2026-05-25.md) §5. |
