...
**Status:** Active spike plan. Priority A (`92b893b`), Priority B (`444fada`), **Priority C semantic-ABI completion** (`e62bf92`), **Item 1 per-library generation infrastructure**, Iteration 2 config unification, Item 2 Image verification, Item 3 GFX, Item 4 TTF, and Item 5 Mixer are closed. See [`priority-c-closure-summary.md`](priority-c-closure-summary.md) and [`satellite-expansion-roadmap.md`](satellite-expansion-roadmap.md). **Next forward scope: Layer 2 public typed low-level API planning** across Core, Image, GFX, TTF, and Mixer. Slice 5 (ppy orchestrator feature parity) remains deferred. Windows-local ClangSharp uses `--use-platform-header-shims` for synthetic Linux/macOS/iOS views; native generation already wired for `x64-linux-hybrid` via the binding-generator docker container. ppy reference analysis recorded in [`ppy-reference-analysis-2026-05-25.md`](ppy-reference-analysis-2026-05-25.md). Do not make a final ClangSharp-vs-CppAst recommendation until comparable CppAst/Alimer-style evidence exists.
...
| **Item 3: SDL2_gfx Layer 1** | ✅ closed 2026-05-27 | GFX generated and active in aggregate output. `FPSmanager` layout and no-SDLCALL Cdecl behavior verified; solution build/oracle/self-tests/slopwatch clean in the Item 3 closure run. |
| **Item 4: SDL2_ttf Layer 1** | ✅ closed 2026-05-27 | TTF generated from one header (`SDL_ttf.h`) and active in aggregate output. Owner-mode `Handles.g.cs` emits `TTF_Font`; Modern uses `CLong`; Compat uses C `long` dual-dispatch; no-SDLCALL functions use Cdecl; deprecated/error macro symbols are absent. Solution build 0/0, oracle TTF raw ABI checks 0 after the oracle helper false-positive fix, Python self-test PASS, C# postprocess self-test PASS, Slopwatch 0 issues. |
| **Item 5: SDL2_mixer Layer 1** | ✅ closed 2026-05-28 | Mixer generated and active in aggregate output. `Mix_Music` owner-mode handle, `Mix_Chunk` transparent struct, callback typedef/function-pointer shape, enum flags shape, error macro exclusions, and value-like legacy version aliases verified. Runtime callback/audio smoke remains deferred. |
| Layer 2 — typed public API | next | Plan public typed low-level API across all five stable Layer 1 families (Core + Image + GFX + TTF + Mixer) before adding friendly overloads. |
| 6+ — Friendly overloads | ⏳ Roadmap M6 | Out of scope until Layer 2 lands. |

## Current Evidence Snapshot — 2026-05-28 (Item 5 closure)
...
- Regeneration result: Core + Image + GFX + TTF + Mixer generated and active. 7-step conceptual postprocess pipeline: `platform-delta` → `strip-varargs` → `libraryimport` (Modern only) → `flags-detect` → `guid-substitute` → `clong-dispatch` → `uniform-opaque`.
- Build command: `dotnet build spikes/binding-generators/clangsharp/Janset.SDL2.ClangSharpSpike.slnx -c Release`.
- Build result: solution compile clean with 0 warnings / 0 errors across the spike solution.
- Oracle evidence: `dotnet run --file spikes/binding-generators/clangsharp/oracle.cs -- --family sdl2-core --family sdl2-image --family sdl2-gfx --family sdl2-ttf --family sdl2-mixer --write-report` writes [`../output/reports/oracle-evidence-clangsharp.md`](../output/reports/oracle-evidence-clangsharp.md) and produces real generated evidence for Mixer.
- Item 5 shape evidence: `SDL_mixer.h` is the only Mixer header; `Generated/{Compat,Modern}/Handles.g.cs` contains `Mix_Music` in `namespace SDL2.Mixer`; `Mix_Chunk` stays transparent with `byte* abuf`; all 6 callback typedefs are represented; all 7 callback-consuming functions are present; `MIX_InitFlags` has `[Flags]`; `Mix_Fading` and `Mix_MusicType` do not; `MIX_MAJOR_VERSION`, `MIX_MINOR_VERSION`, and `MIX_PATCHLEVEL` are generated; `Mix_SetError`, `Mix_GetError`, `Mix_ClearError`, and `Mix_OutOfMemory` are absent.
- Peer visual evidence: SDL2-CS uses `IntPtr` for `Mix_Music` and `MIX_Chunk.abuf`, while Janset intentionally emits typed `Mix_Music` and pointer-shaped `byte* abuf`; SDL2-CS callback delegates use `[UnmanagedFunctionPointer(CallingConvention.Cdecl)]`, matching Compat; Modern uses `delegate* unmanaged[Cdecl]<...>`. SDL2-CS constants are older than pinned SDL2_mixer 2.8.1. ppy/SDL3-CS was used only for ClangSharp callback-shape evidence, not as an SDL2_mixer API oracle. No useful Silk.NET SDL2_mixer peer surface was found.
- Self-tests: Python self-test PASS and C# postprocess self-test PASS.
- Existing Core runtime ABI smoke remains the last recorded runtime test baseline: `dotnet test --project spikes/binding-generators/clangsharp/tests/abi-tests/AbiTests.csproj -c Release` passes `792/792` across `net462`, `net8.0`, `net9.0`, and `net10.0` on Windows x64. TTF-specific runtime smoke is deferred until a redistributable font asset and native copy strategy are in scope; Mixer callback/audio smoke is deferred until callback lifetime/rooting policy, dummy-audio setup, and cleanup rules are designed. Full 7-RID proof remains a production CI gate.
...
| Mixer callback lifetime/rooting policy and deterministic callback smoke | SDL2_mixer callback registrations can outlive the immediate call, so Layer 1 should only prove raw callback signatures. Runtime callback smoke needs explicit Compat delegate rooting, Modern `delegate* unmanaged[Cdecl]` callback shape, dummy-audio setup, and cleanup rules. | Layer 2/Layer 3 callback policy plus follow-up AbiTests or package smoke after generated callbacks are promoted. |
| Runtime-endian public aliases for audio/pixel defaults | `MIX_DEFAULT_FORMAT` currently emits as `0x8010` because `AUDIO_S16SYS` was evaluated on a little-endian generator host. This is correct for all current RIDs but less portable than SDL2-CS/ppy-style runtime expressions. | Consider public typed API aliases/properties for `AUDIO_*SYS` / `MIX_DEFAULT_FORMAT`, plus equivalent policy for `SDL_PIXELFORMAT_*32`. See [satellites/sdl2-endian-platform-macro-consolidation.md](satellites/sdl2-endian-platform-macro-consolidation.md). |
