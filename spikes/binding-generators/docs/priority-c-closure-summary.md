...
- **5-pass postprocess pipeline** (`postprocess/Program.cs`): `platform-delta` → `strip-varargs` → `libraryimport` (Modern only) → `guid-substitute` → `threadid-dispatch` → `uniform-opaque`.
