...
internal static unsafe class SdlMacro
{
    public static SDL_Surface* LoadBmp(string path)
    {
        ...
        SDL_RWops source = SDLNative.SDL_RWFromFile(pinnedPath.Pointer, readMode.Pointer);
        if (source.IsNull)
        {
        ...
        SDL_Surface* surface = SDLNative.SDL_LoadBMP_RW(source, 1);
        if (surface is null)
        {
    ...
    public static int SaveBmp(SDL_Surface* surface, string path)
    {
        ...
        SDL_RWops destination = SDLNative.SDL_RWFromFile(pinnedPath.Pointer, writeMode.Pointer);
        ...
        return SDLNative.SDL_SaveBMP_RW(surface, destination, 1);
    }
}
